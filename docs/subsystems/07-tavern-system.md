...
酒馆等级是游戏的**核心元进度**。声望 (Reputation) 通过完成冒险累积，解锁新设施与功能。

| 等级 | 名称 | XP阈值 | 累计声望XP | 解锁区域 | 解锁功能 | 视觉变化 |
|:----:|------|:------:|:---------:|----------|----------|----------|
| Lv1 | 破旧小屋 | 0 | 0 | 大厅 (Hall) | 基础招募、基础任务板、基础休息 | 小木屋，吧台一张，壁炉破损 |
| Lv2 | 路边客栈 | 1,200 | 1,200 | 铁匠铺 (Blacksmith) | 装备修复、基础武器/护甲打造、基础商店 | 木质招牌挂起，铁砧搬入后院 |
| Lv3 | 繁荣旅店 | 2,500 | 3,700 | 炼金台 (Alchemist) | 药水/卷轴制作、炼金材料商店 | 二楼加盖，壁炉翻新，吧台扩建 |
| Lv4 | 知名驿站 | 5,000 | 8,700 | — | 中冒险解锁、任务板扩容、角色槽位+1 | 石墙加固，大厅悬挂冒险者战利品 |
| Lv5 | 冒险者大厅 | 9,000 | 17,700 | 图书馆 (Library) | 法术卷轴、技能训练、新职业解锁、专长学习 | 地下室扩建为图书室，书架装满古籍 |
| Lv6 | 英雄驿站 | 15,000 | 32,700 | — | 角色关系系统深化、酒馆事件系统启用、角色槽位+1 | 花园扩建，NPC休闲区建立 |
| Lv7 | 圣殿驿站 | 24,000 | 56,700 | 神殿 (Temple) | 角色复活、诅咒移除、祝福buff | 侧翼圣殿建成，彩绘玻璃窗 |
| Lv8 | 传说枢纽 | 38,000 | 94,700 | — | 长冒险解锁、传奇任务线开启、角色槽位+1 | 英雄之壁揭开，前代冒险者雕塑 |
| Lv9 | 命运交汇 | 60,000 | 154,700 | — | 双任务同时发布、英雄传记完整功能、Prestige系统 | 整栋建筑扩建为三层石堡，魔法灯笼悬浮 |
| Lv10 | 酒馆与命运 | 100,000 | 254,700 | 英雄之壁完整功能 | 传说装备解锁、全功能巅峰、传奇冒险、角色槽位满 | 金碧辉煌的大厅，壁炉上方悬挂历代英雄画像 |

### 2.2 声望XP获取规则

| 来源 | 声望XP | 说明 |
|------|:------:|------|
| 完成短冒险 | 200-400 XP | 基础值300，难度系数修正 |
| 完成中冒险 | 500-1,200 XP | 基础值800，难度系数修正 |
| 完成长冒险 | 1,500-4,000 XP | 基础值2,500，难度系数修正 |
| Boss击杀 | 100-500 XP | 根据Boss CR |
| 角色达到Lv10 | 500 XP | 每角色一次 |
| 角色达到Lv20 | 2,000 XP | 每角色一次 |
| 解锁新区域 | 200 XP | 每区域一次 |
| 英雄之壁新增传记 | 100 XP | 每次 |
...
```json
{
  ...
  "visual_data": {
    "background_path": "res://assets/backgrounds/tavern_blacksmith.png",
...
```
TavernScene (Node2D)
├── TavernManager (Autoload Singleton)              # 酒馆全局管理器
│
├── BackgroundLayer (CanvasLayer)
│   └── TavernBackground (Sprite2D)                 # 酒馆全景背景
│
├── AreaManager (Node)                               # 区域管理器
│   ├── AreaHall (Node2D)                           # 大厅
│   │   ├── HallBackground (Sprite2D)
│   │   ├── RecruitmentBoard (InteractableObject)   # 招募板
│   │   ├── QuestBoard (InteractableObject)         # 任务板
│   │   ├── BarkeepNPC (NPCCharacter)               # 酒馆老板
│   │   ├── LoungeCharacters (Node2D)               # 闲逛角色
│   │   │   ├── CharacterSprite_1
│   │   │   ├── CharacterSprite_2
│   │   │   └── ...
│   │   └── RotatingNPCs (Node2D)                   # 轮换NPC
│   │
│   ├── AreaBlacksmith (Node2D)                     # 铁匠铺
│   │   ├── BlacksmithBackground
│   │   ├── BlacksmithNPC
│   │   ├── Anvil (InteractableObject)
│   │   └── Forge (InteractableObject)
│   │
│   ├── AreaAlchemist (Node2D)                      # 炼金台
│   ├── AreaLibrary (Node2D)                        # 图书馆
│   ├── AreaTemple (Node2D)                         # 神殿
│   └── AreaHeroesWall (Node2D)                     # 英雄之壁
│
├── TavernUI (CanvasLayer)                           # UI层
│   ├── TopBar (Control)
│   │   ├── GoldDisplay (Label)
│   │   ├── TavernLevelDisplay (Label)
│   │   └── ReputationXPBar (ProgressBar)
│   │
│   ├── RosterPanel (Control)                       # 角色名册面板
│   ├── CharacterSheet (Control)                    # 角色详情面板
│   ├── ShopUI (Control)                            # 商店面板
│   ├── RecruitmentUI (Control)                     # 招募面板
│   ├── QuestBoardUI (Control)                      # 任务板面板
│   ├── DialogueUI (Control)                        # 对话面板
│   ├── EventUI (Control)                           # 事件面板
│   ├── TrainingUI (Control)                        # 训练面板
│   └── RelationshipNetworkUI (Control)             # 关系网络面板
│
├── TavernEventManager (Node)                       # 事件管理器
├── NPCScheduler (Node)                             # NPC调度器
├── ShopManager (Node)                              # 商店管理器
├── RestManager (Node)                              # 休息管理器
├── TrainingManager (Node)                          # 训练管理器
└── TavernDialogueManager (Node)                    # 对话管理器
```

### 13.2 TavernManager 单例 (Autoload)

```gdscript
# autoload/tavern_manager.gd
class_name TavernManager
extends Node

## TavernManager — 酒馆系统全局单例
## 管理酒馆等级、声望、区域状态、角色名册

# === 酒馆状态 ===
var tavern_level: int = 1
var reputation_xp: int = 0
var prestige_level: int = 0
var tavern_gold: int = 100  # 酒馆金库 (用于升级)

# === 区域状态 ===
var unlocked_areas: Dictionary = {
    "area_hall": true,
    "area_blacksmith": false,
    "area_alchemist": false,
    "area_library": false,
    "area_temple": false,
    "area_heroes_wall": false
}
...
# === 当前冒险队伍 (从roster中选出的4人) ===
var party: Array[CharacterResource] = []

# === 信号 ===
signal tavern_leveled_up(old_level: int, new_level: int)
signal area_unlocked(area_id: String)
signal reputation_xp_changed(new_xp: int, delta: int)
signal roster_changed()
signal party_changed()
signal character_recruited(character_id: String)
signal character_dismissed(character_id: String)
signal tavern_event_triggered(event_id: String)
signal npc_arrived(npc_id: String, npc_type: String)
...
func unlock_area(area_id: String) -> bool:
    if unlocked_areas.get(area_id, false):
        return false
    var area_data = AreaDatabase.get_area(area_id)
    if tavern_level < area_data.unlock_tavern_level:
        return false
    if tavern_gold < area_data.unlock_cost_gold:
        return false
    tavern_gold -= area_data.unlock_cost_gold
...
func add_reputation_xp(amount: int) -> void:
    var old_xp = reputation_xp
    reputation_xp += amount
    reputation_xp_changed.emit(reputation_xp, amount)
    check_level_up()

func check_level_up() -> void:
    var level_thresholds = [0, 1200, 3700, 8700, 17700, 32700, 56700, 94700, 154700, 254700]
    for i in range(tavern_level, min(level_thresholds.size(), 11)):
        if tavern_level < 10 and reputation_xp >= level_thresholds[tavern_level]:
            var old = tavern_level
            tavern_level += 1
            max_roster_size = get_roster_size_for_level(tavern_level)
            tavern_leveled_up.emit(old, tavern_level)
        else:
            break

# === 角色管理 ===
func recruit_character(character: CharacterResource) -> bool:
    if roster.size() >= max_roster_size:
        return false
    roster.append(character)
    character_recruited.emit(character.character_id)
    roster_changed.emit()
    return true

func dismiss_character(character_id: String) -> bool:
    for i in range(roster.size()):
        if roster[i].character_id == character_id:
            roster[i].status = "dismissed"
            roster.remove_at(i)
            character_dismissed.emit(character_id)
            roster_changed.emit()
            return true
    return false

func set_party(characters: Array[CharacterResource]) -> bool:
    if characters.size() > 4 or characters.size() < 1:
        return false
    party = characters
    party_changed.emit()
    return true
...
# === Helpers ===
func get_roster_size_for_level(level: int) -> int:
    match level:
        1, 2, 3: return 6
        4, 5: return 7
        6, 7: return 9
...
```gdscript
# res://scripts/tavern/area_manager.gd
class_name AreaManager
extends Node

## 管理酒馆所有区域的加载、显示、交互

var areas: Dictionary = {}  # area_id -> AreaNode
var current_area_id: String = "area_hall"

signal area_changed(from_id: String, to_id: String)
signal area_interaction(area_id: String, object_id: String)

func _ready() -> void:
    _initialize_areas()

func _initialize_areas() -> void:
    for area_id in TavernManager.unlocked_areas:
        if TavernManager.unlocked_areas[area_id]:
            _load_area(area_id)

func _load_area(area_id: String) -> void:
    var area_scene = load("res://scenes/tavern/areas/" + area_id + ".tscn")
    var area_node = area_scene.instantiate()
    add_child(area_node)
    areas[area_id] = area_node
    if area_id != "area_hall":
        area_node.visible = false

func switch_area(area_id: String) -> void:
    if not TavernManager.unlocked_areas.get(area_id, false):
        return
    areas[current_area_id].visible = false
    current_area_id = area_id
    areas[current_area_id].visible = true
    area_changed.emit(current_area_id, area_id)
```

### 13.4 NPC管理类

```gdscript
# res://scripts/tavern/npc_scheduler.gd
class_name NPCScheduler
extends Node

## NPC调度器 — 管理NPC的到达、活动和离开

var active_npcs: Array[Dictionary] = []
var npc_history: Array[Dictionary] = []
var max_concurrent_npcs: int = 3

signal npc_arrived(npc_id: String, area_id: String)
signal npc_departed(npc_id: String)
signal npc_interaction_available(npc_id: String)

func check_schedule(trigger: String = "on_return") -> void:
    # 检查是否需要新NPC
    if active_npcs.size() < max_concurrent_npcs:
        var candidate = _select_candidate()
        if candidate:
            _spawn_npc(candidate)

func _select_candidate() -> Dictionary:
    var candidates = _get_available_npcs()
    var total_weight = 0
    for c in candidates:
        total_weight += c.get("appearance_weight", 10)
    ...
    var cumulative = 0
    for c in candidates:
        cumulative += c.get("appearance_weight", 10)
        if roll < cumulative:
            return c
    return {}

func _get_available_npcs() -> Array:
    var pool = NPCDataPool.get_rotating_npcs()
    var available: Array = []
    
    for npc_data in pool:
        # 检查冷却时间
        if _is_on_cooldown(npc_data["npc_id"]):
            continue
        # 检查酒馆等级条件
        if TavernManager.tavern_level < npc_data.get("min_tavern_level", 1):
            continue
        # 检查并发上限
        var same_type_count = 0
        for active in active_npcs:
            if active.get("type") == npc_data.get("type"):
                same_type_count += 1
        if same_type_count >= npc_data.get("max_concurrent", 1):
            continue
        available.append(npc_data)
    
    return available

func _spawn_npc(npc_data: Dictionary) -> void:
    var npc_instance = NPCFactory.create_npc(npc_data)
    var area_id = npc_data.get("area", "area_hall")
    active_npcs.append({
        "npc_id": npc_data["npc_id"],
        "instance": npc_instance,
        "area_id": area_id,
        "visit_start": Time.get_datetime_string_from_system(),
        "visit_duration": npc_data.get("visit_duration_adventures", 1),
        "data": npc_data
    })
    npc_arrived.emit(npc_data["npc_id"], area_id)
    
    # LLM生成NPC到达场景文本
    _generate_arrival_narrative(npc_data)

func _generate_arrival_narrative(npc_data: Dictionary) -> void:
    var prompt = "NPC到达酒馆: {name} ({role}), {type}.\n生成1-2句到达场景描写。(中文)".format(npc_data)
    LLMGateway.request_copywriter_text(prompt, _on_arrival_text_ready)

func _on_arrival_text_ready(text: String) -> void:
    EventUI.show_narrative_popup(text, duration:=4.0)
```

### 13.5 事件管理类

```gdscript
# res://scripts/tavern/tavern_event_manager.gd
class_name TavernEventManager
extends Node

## 酒馆事件管理器 — 管理事件池、触发条件、结果处理

var event_pool: Dictionary = {}  # 加载自 tavern_event_pool.json
var event_cooldowns: Dictionary = {}  # event_id -> last_triggered_adventure_id
var event_history: Array[Dictionary] = []  # 最近触发的事件

signal event_triggered(event_id: String, event_data: Dictionary)
signal event_resolved(event_id: String, outcome_id: String, mechanical_result: Dictionary)

func _ready() -> void:
    _load_event_pool()

func _load_event_pool() -> void:
    var file = FileAccess.open("res://data/tavern/tavern_event_pool.json", FileAccess.READ)
    event_pool = JSON.parse_string(file.get_as_text())

func check_and_trigger(trigger_type: String) -> void:
    var base_chance = _get_base_chance(trigger_type)
    var modifiers = _calculate_modifiers(trigger_type)
    var final_chance = clamp(base_chance + modifiers, 0, 100)
    
    if randi() % 100 < final_chance:
        var event = _select_event()
        if event:
            trigger_event(event)

func _select_event() -> Dictionary:
    var candidates = []
    var total_weight = 0
    
    for event in event_pool.get("events", []):
    ...
    for event in candidates:
        cumulative += event.get("weight", 10)
        if roll < cumulative:
            return event
    return {}
...
func _can_trigger(event: Dictionary) -> bool:
    # 检查酒馆等级
    if TavernManager.tavern_level < event.get("min_tavern_level", 1):
        return false
    # 检查冷却时间
    var event_id = event["event_id"]
    if event_id in event_cooldowns:
        var last_trigger = event_cooldowns[event_id]
        var current = TavernManager.adventures_completed
        var cooldown = event.get("cooldown_adventures", 1)
        if current - last_trigger < cooldown:
            return false
    # 检查禁止条件
    for prohibited in event.get("triggers", {}).get("prohibited_if", []):
        if _check_condition(prohibited):
            return false
    # 检查参与者可用性
    var participants = event.get("participants", {})
    if participants.get("count", 0) > 0:
        var available = _select_participants(participants)
        if available.size() < participants.get("count", 1):
            return false
    return true

func trigger_event(event: Dictionary) -> void:
    event_triggered.emit(event["event_id"], event)
    event_cooldowns[event["event_id"]] = TavernManager.adventures_completed
    
    # 选择参与者
    var participants = _select_participants(event.get("participants", {}))
    
    # 调用LLM生成事件叙事
    var prompt = event.get("llm_prompt_template", "")
    for participant in participants:
        prompt = prompt.replace("{" + participant["role"] + "}", participant["name"])
    
    # 构建LLM请求
    var llm_request = {
        "request_type": "tavern_event",
    ...
    # 展示事件UI
    TavernUI.show_event_ui(event, participants, llm_request)

func resolve_event(event_id: String, outcome_id: String) -> void:
    var event = _find_event(event_id)
    var outcome = _find_outcome(event, outcome_id)
    
    if outcome:
        # 应用机械结果
        var mechanical = outcome.get("mechanical_effect", {})
        _apply_mechanical_result(mechanical)
        event_resolved.emit(event_id, outcome_id, mechanical)

func _apply_mechanical_result(result: Dictionary) -> void:
    if result.has("relationship_change"):
        # 应用关系变化
        RelationshipSystem.apply_change(
            result.get("character_a_id", ""),
            result.get("character_b_id", ""),
            result["relationship_change"]
        )
    if result.has("gold_change"):
        TavernManager.tavern_gold += result["gold_change"]
    if result.has("buff_id"):
        # 应用buff
        BuffSystem.apply_tavern_buff(result["buff_id"], result.get("duration", "next_adventure"))
    if result.get("generate_side_quest"):
        QuestBoardManager.generate_character_quest(result.get("character_id", ""))
```

### 13.6 信号定义

```gdscript
# res://scripts/signals/tavern_signals.gd
extends Node

## 酒馆系统相关信号总线

# === 酒馆状态 ===
signal tavern_level_up(old_level: int, new_level: int)
signal reputation_xp_changed(current: int, delta: int)
signal prestige_level_up(new_level: int)
signal tavern_gold_changed(new_amount: int)

# === 区域 ===
signal area_entered(area_id: String)
signal area_exited(area_id: String)
signal area_unlocked(area_id: String)
signal facility_used(area_id: String, facility_id: String)

# === 角色管理 ===
signal roster_updated(roster: Array)
signal character_recruited(character_id: String)
signal character_dismissed(character_id: String)
signal party_selected(characters: Array)
signal party_ready_for_adventure()

# === 招募板 ===
signal recruitment_board_refreshed(candidates: Array)
signal recruitment_cost_calculated(character_id: String, cost: int)

# === NPC ===
signal npc_arrived_in_tavern(npc_id: String, npc_type: String)
signal npc_departed_from_tavern(npc_id: String)
signal npc_dialogue_started(npc_id: String, dialogue_tree_id: String)
signal npc_dialogue_ended(npc_id: String)

# === 商店 ===
signal shop_opened(shop_id: String, inventory: Array)
signal shop_closed(shop_id: String)
signal item_purchased(item_id: String, price: int)
signal item_sold_to_shop(item_id: String, price: int)

# === 事件 ===
signal tavern_event_triggered(event_id: String, participants: Array)
signal tavern_event_resolved(event_id: String, outcome_id: String)
signal relationship_event_triggered(type: String, chars: Array)
signal relationship_threshold_crossed(character_a: String, character_b: String, new_label: String)

# === 训练 ===
signal training_started(character_id: String, training_type: String)
signal training_completed(character_id: String, training_type: String, result: Dictionary)
signal skill_proficiency_gained(character_id: String, skill: String)
signal feat_learned(character_id: String, feat_id: String)

# === 休息 ===
signal short_rest_completed()
signal long_rest_completed()
signal hit_dice_used(character_id: String, remaining: int)

# === 任务板 ===
signal quest_board_refreshed(quests: Array)
signal quest_accepted(quest_id: String)
signal adventure_departed(quest_id: String, party: Array)

# === 对话 ===
signal dialogue_node_visited(npc_id: String, node_id: String)
signal dialogue_choice_made(npc_id: String, choice_id: String)
signal dialogue_check_performed(check_type: String, success: bool)
```
...
├── scripts/
│   ├── tavern/
│   │   ├── tavern_manager.gd              # TavernManager 单例 (Autoload)
│   │   ├── area_manager.gd               # 区域管理器
│   │   ├── tavern_event_manager.gd       # 事件管理器
│   │   ├── npc_scheduler.gd              # NPC调度器
│   │   ├── shop_manager.gd               # 商店管理器
│   │   ├── recruitment_manager.gd        # 招募管理器
│   │   ├── quest_board_manager.gd        # 任务板管理器
│   │   ├── rest_manager.gd              # 休息管理器
│   │   ├── training_manager.gd          # 训练管理器
│   │   ├── tavern_dialogue_manager.gd    # 对话管理器
│   │   ├── relationship_event_manager.gd # 关系事件管理器
│   │   │
│   │   ├── data/
│   │   │   ├── area_database.gd          # 区域数据库 (AreaData resources)
│   │   │   ├── npc_data_pool.gd          # NPC数据池
│   │   │   ├── event_pool_loader.gd      # 事件池加载器
│   │   │   ├── dialogue_tree_loader.gd   # 对话树加载器
│   │   │   └── shop_data_loader.gd       # 商店数据加载器
│   │   │
│   │   ├── ui/
│   │   │   ├── tavern_ui_controller.gd   # 酒馆UI总控制器
│   │   │   ├── roster_panel.gd          # 角色名册面板
│   │   │   ├── character_sheet.gd       # 角色详情面板
│   │   │   ├── recruitment_ui.gd        # 招募面板
│   │   │   ├── shop_ui.gd              # 商店面板
│   │   │   ├── quest_board_ui.gd        # 任务板面板
│   │   │   ├── dialogue_ui.gd          # 对话面板
│   │   │   ├── event_ui.gd             # 事件面板
│   │   │   ├── training_ui.gd          # 训练面板
│   │   │   ├── relationship_network_ui.gd # 关系网络面板
│   │   │   └── tavern_hud.gd           # 酒馆HUD (顶部状态栏)
│   │   │
│   │   └── entities/
│   │       ├── npc_character.gd          # NPC角色实体
│   │       ├── interactable_object.gd    # 可交互物体基类
│   │       ├── recruitment_board.gd      # 招募板实体
│   │       └── quest_board.gd           # 任务板实体
│   │
│   └── signals/
│       └── tavern_signals.gd            # 酒馆信号总线
│
├── scenes/
│   └── tavern/
│       ├── tavern_scene.tscn             # 酒馆主场景
│       ├── areas/
│       │   ├── area_hall.tscn
│       │   ├── area_blacksmith.tscn
│       │   ├── area_alchemist.tscn
│       │   ├── area_library.tscn
│       │   ├── area_temple.tscn
│       │   └── area_heroes_wall.tscn
│       └── ui/
│           ├── roster_panel.tscn
...
│           ├── event_ui.tscn
│           ├── training_ui.tscn
│           └── relationship_network_ui.tscn
│
├── data/
│   └── tavern/
│       ├── tavern_level_progression.json  # 酒馆等级进度表
│       ├── area_definitions.json        # 区域数据定义
│       ├── tavern_event_pool.json       # 事件池数据
│       ├── npc_pool.json               # NPC数据池
│       ├── dialogue_trees/              # 对话树数据
│       │   ├── dt_barkeep_maria.json
│       │   ├── dt_blacksmith_grim.json
│       │   └── ...
│       ├── shop_data.json              # 商店数据
│       ├── training_costs.json          # 训练费用表
│       ├── recruitment_config.json      # 招募配置
│       └── quest_board_config.json      # 任务板配置
│
├── assets/
│   └── backgrounds/
│       └── tavern/
│           ├── tavern_hall.png          # 大厅背景
│           ├── tavern_blacksmith.png    # 铁匠铺背景
│           ├── tavern_alchemist.png     # 炼金台背景
│           ├── tavern_library.png       # 图书馆背景
│           ├── tavern_temple.png        # 神殿背景
│           └── tavern_heroes_wall.png   # 英雄之壁背景
│
└── tests/
    └── tavern/
        ├── unit/
        │   ├── test_tavern_level_progression.gd
        │   ├── test_recruitment_pool_generation.gd
        │   ├── test_shop_transactions.gd
        │   ├── test_event_trigger_conditions.gd
        │   ├── test_relationship_threshold.gd
        │   ├── test_dialogue_tree_traversal.gd
        │   └── test_training_costs.gd
        └── integration/
            ├── test_full_tavern_flow.gd
            ├── test_recruit_to_party_to_adventure.gd
            └── test_event_chain.gd
```
