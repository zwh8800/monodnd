...
本系统是连接"创意层"（LLM Agent）与"执行层"（游戏引擎）的桥梁——LLM负责"写什么故事"，程序负责"怎么把这个故事变成可以玩的游戏"。
...
Step 1: 加载对应tier的基础模板
  template = load("res://data/adventure_templates/template_%s.json" % tier)
...
├─────────────────────────────────────────────────┤
│ Step 5: 更新世界状态 (负面变更)                   │
│  · 执行world_state_hooks.on_failure              │
│  · 区域威胁等级+1                                │
│  · 势力关系可能恶化                              │
├─────────────────────────────────────────────────┤
│ Step 6: 生成失败叙事 (LLM)                       │
...
参照 GDD §6.1 伤疤设计和 character-system.md 中的伤疤数据模型：

```json
{
  "scar_id": "scar_fire_trauma",
  "name": "惧焰",
  ...
  "trigger_context": "受到火焰伤害时",
  "visual_indicator": "角色精灵左臂有烧伤痕迹"
}
```
...
#### AdventureManager — 主控制器

```gdscript
# autoload/adventure_manager.gd
extends Node

## AdventureManager — 冒险生成系统主控制器
## 管理冒险的完整生命周期: 生成 → 实例化 → 进行中 → 结算

# 状态
enum AdventurePhase { IDLE, GENERATING, INSTANTIATING, IN_PROGRESS, SETTLING, COMPLETED }
var current_phase: AdventurePhase = AdventurePhase.IDLE
var current_adventure: AdventureInstance = null
var adventure_state: AdventureState = null

# 组件引用
var blueprint_parser: BlueprintParser
var instantiator: AdventureInstantiator
var state_manager: AdventureStateManager
var encounter_generator: EncounterGenerator
var loot_generator: LootGenerator
var settlement_system: SettlementSystem

# 信号
signal adventure_generation_started(tier: String)
signal blueprint_received(blueprint: Dictionary)
signal blueprint_validated(success: bool, errors: Array)
signal adventure_instantiated(instance: AdventureInstance)
signal adventure_started(adventure_id: String)
signal adventure_completed(adventure_id: String, outcome: String)
signal adventure_failed(adventure_id: String, reason: String)
signal node_entered(node_id: String, node_type: String)
signal choice_made(choice_id: String, outcome: String)
signal encounter_triggered(encounter_id: String)
signal loot_dropped(items: Array)

func start_adventure(tier: String, theme_pool: Array = []) -> void:
    """开始冒险生成流程"""
    current_phase = AdventurePhase.GENERATING
    adventure_generation_started.emit(tier)

    # 构建编剧Agent输入
    var input = _build_screenwriter_input(tier, theme_pool)

    # 调用编剧Agent
    var response = await LLMGateway.submit_request(
        ScreenwriterAgent.create_request(input)
    )

    if response.status == AgentResponse.Status.SUCCESS:
        var blueprint = response.data
        blueprint_received.emit(blueprint)

        # 验证蓝图
        var validation = blueprint_parser.validate(blueprint)
        blueprint_validated.emit(validation.is_valid, validation.errors)

        if validation.is_valid:
            # 调用平衡Agent
            var balance_report = await _run_balancer(blueprint)
            if balance_report.overall_assessment.status != "rejected":
                # 应用平衡调整
                blueprint = _apply_balance_adjustments(blueprint, balance_report)

                # 实例化
                current_phase = AdventurePhase.INSTANTIATING
                current_adventure = await instantiator.instantiate(blueprint, _get_party_state())
                adventure_instantiated.emit(current_adventure)

                # 开始冒险
                current_phase = AdventurePhase.IN_PROGRESS
                adventure_state = state_manager.initialize(blueprint, current_adventure)
                adventure_started.emit(blueprint.id)
            else:
                _fallback_to_template(tier)
        else:
            _retry_or_fallback(blueprint, validation.errors, tier)
    else:
        _fallback_to_template(tier)

func enter_node(node_id: String) -> void:
    """进入新节点"""
    adventure_state.current_node_id = node_id
    adventure_state.visited_nodes.append(node_id)
    node_entered.emit(node_id, _get_node_type(node_id))

    # 触发DM Agent场景描写
    _request_scene_atmosphere(node_id)

    # 检查节点类型并触发相应逻辑
    var node_data = current_adventure.get_node(node_id)
    match node_data.type:
        "combat", "elite_combat", "ambush":
            _trigger_encounter(node_data)
        "dialogue":
            _start_dialogue(node_data)
        "exploration":
            _enter_exploration(node_data)
        "puzzle":
            _start_puzzle(node_data)
        "merchant":
            _open_merchant(node_data)
        "rest":
            _enter_rest(node_data)
        "boss":
            _trigger_boss_encounter(node_data)
        "branch":
            _present_branch_choices(node_data)
        "epilogue":
            _start_settlement()

func make_choice(choice_id: String) -> void:
    """玩家做出选择"""
    var node_data = current_adventure.get_node(adventure_state.current_node_id)
    var choice = _find_choice(node_data, choice_id)
    var result = _resolve_choice(choice, adventure_state, _get_party_state())

    choice_made.emit(choice_id, "success" if result.success else "failed")

    if result.success and result.target_node:
        enter_node(result.target_node)

func complete_adventure(outcome: String) -> void:
    """完成冒险"""
    current_phase = AdventurePhase.SETTLING
    await settlement_system.settle(current_adventure, adventure_state, outcome)
    current_phase = AdventurePhase.COMPLETED
    adventure_completed.emit(adventure_state.adventure_id, outcome)
```

#### BlueprintParser — 蓝图解析器

```gdscript
# scripts/adventure/blueprint_parser.gd
class_name BlueprintParser
extends Node

## 蓝图解析器 — 负责JSON解析、Schema验证、业务逻辑验证

var schema_validator: SchemaValidator
var business_validator: BlueprintBusinessValidator

func validate(blueprint: Dictionary) -> ValidationResult:
    """完整验证流程"""
    var result = ValidationResult.new()

    # Step 1: Schema验证
    var schema_result = schema_validator.validate(blueprint, "adventure_blueprint")
    if not schema_result.is_valid:
        result.is_valid = false
        result.errors.append_array(schema_result.errors)
        return result

    # Step 2: 业务逻辑验证
    var business_errors = business_validator.validate(blueprint, _get_party_state())
    var critical_errors = business_errors.filter(func(e): return e.severity == "error")
    var warnings = business_errors.filter(func(e): return e.severity == "warning")

    if not critical_errors.is_empty():
        result.is_valid = false
        result.errors.append_array(critical_errors)

    result.warnings = warnings
    return result

func parse(blueprint: Dictionary) -> ParsedBlueprint:
    """解析蓝图为内部数据结构"""
    return ParsedBlueprint.new({
        "id": blueprint.id,
        "meta": blueprint.meta,
        "nodes": _parse_nodes(blueprint.plot_outline.nodes),
        "npcs": _parse_npcs(blueprint.key_npcs),
        "encounters": _parse_encounters(blueprint.encounters),
        "difficulty": blueprint.difficulty_profile,
        "loot": blueprint.loot_profile,
        "world_hooks": blueprint.world_state_hooks,
        "theme_tags": blueprint.theme_tags
    })
```

#### AdventureInstantiator — 实例化引擎

```gdscript
# scripts/adventure/adventure_instantiator.gd
class_name AdventureInstantiator
extends Node

## 实例化引擎 — 将解析后的蓝图转化为可玩内容

var encounter_generator: EncounterGenerator
var loot_generator: LootGenerator
var room_template_loader: RoomTemplateLoader

signal instantiation_progress(step: int, total_steps: int, description: String)

func instantiate(blueprint: ParsedBlueprint, party_state: Dictionary) -> AdventureInstance:
    """14步实例化流程"""
    var instance = AdventureInstance.new()
    var total_steps = 14

    # Step 1: 解析+验证 (已完成)
    instantiation_progress.emit(1, total_steps, "蓝图验证完成")

    # Step 2: 生成节点图
    var node_graph = _generate_node_graph(blueprint)
    instantiation_progress.emit(2, total_steps, "节点图生成完成")

    # Step 3: 分配房间模板
    for node_id in node_graph:
        node_graph[node_id].room_template = room_template_loader.select(
            node_graph[node_id].type,
            node_graph[node_id].room_size,
            node_graph[node_id].theme_tags
        )
    instantiation_progress.emit(3, total_steps, "房间模板分配完成")

    # Step 4: 生成战斗遭遇
    for node_id in node_graph:
        var node = node_graph[node_id]
        if node.type in ["combat", "elite_combat", "ambush"]:
            node.encounter_data = encounter_generator.generate(
                node.encounter_id,
                blueprint.difficulty,
                blueprint.theme_tags,
                party_state
            )
    instantiation_progress.emit(4, total_steps, "遭遇数据生成完成")

    # Step 5-10: 其他节点类型配置
    _configure_dialogue_nodes(node_graph, blueprint)
    _configure_exploration_nodes(node_graph, blueprint)
    _configure_merchant_nodes(node_graph, blueprint)
    _configure_rest_nodes(node_graph, blueprint)
    _configure_puzzle_nodes(node_graph, blueprint)
    _configure_boss_nodes(node_graph, blueprint, party_state)
    instantiation_progress.emit(10, total_steps, "所有节点配置完成")

    # Step 11: 连接走廊
    _generate_corridors(node_graph)
    instantiation_progress.emit(11, total_steps, "走廊连接完成")

    # Step 12: 放置交互标签
    _place_interaction_tags(node_graph)
    instantiation_progress.emit(12, total_steps, "交互标签放置完成")

    # Step 13: 设置触发器
    _setup_triggers(node_graph)
    instantiation_progress.emit(13, total_steps, "触发器设置完成")

    # Step 14: 初始化状态
    instance.node_graph = node_graph
    instance.blueprint = blueprint
    instantiation_progress.emit(14, total_steps, "实例化完成")
...
```gdscript
# scripts/adventure/encounter_generator.gd
class_name EncounterGenerator
extends Node

## 遭遇生成器 — 基于CR预算生成敌人组合

func generate(encounter_id: String, difficulty_profile: Dictionary, theme_tags: Array, party_state: Dictionary) -> EncounterData:
    """生成完整遭遇数据"""
    var encounter_blueprint = _find_encounter_blueprint(encounter_id)
    var party_level = party_state.average_level
    var party_size = party_state.members.size()

    # 计算CR预算
    var cr_budget = calculate_cr_budget(party_level, party_size, encounter_blueprint.difficulty)

    # 生成敌人组合
    var enemies = generate_enemy_composition(cr_budget, theme_tags, encounter_blueprint)

    # 选择阵型
    var formation = encounter_blueprint.get("formation", _select_formation(enemies.size()))

    # 生成地形特征
    var terrain = encounter_blueprint.get("terrain_features", [])

    return EncounterData.new({
        "encounter_id": encounter_id,
        "difficulty": encounter_blueprint.difficulty,
        "cr_budget": cr_budget,
        "enemies": enemies,
        "formation": formation,
        "terrain": terrain,
        "trigger_type": encounter_blueprint.get("trigger_type", "proximity"),
        "can_avoid": encounter_blueprint.get("can_avoid", false)
    })

func calculate_cr_budget(party_level: int, party_size: int, difficulty: String) -> float:
    """基于DND 5e规则计算CR预算"""
    var xp_thresholds = {
    ...
    var per_person_xp = xp_thresholds[difficulty].get(party_level, 150)
    var total_xp_budget = per_person_xp * party_size

    # 转换为CR等价 (简化)
    return _xp_to_cr(total_xp_budget)
```

#### AdventureStateManager — 状态管理器

```gdscript
# scripts/adventure/adventure_state_manager.gd
class_name AdventureStateManager
extends Node

## 冒险状态管理器 — 管理冒险运行时状态、存档、日志

func initialize(blueprint: Dictionary, instance: AdventureInstance) -> AdventureState:
    """初始化冒险状态"""
    var state = AdventureState.new()
    state.adventure_id = blueprint.id
    state.adventure_title = blueprint.meta.title
    state.tier = blueprint.meta.tier
    state.current_node_id = _find_start_node(instance.node_graph)
    state.visited_nodes = [state.current_node_id]
    state.start_time = Time.get_unix_time_from_system()
    return state

func save(state: AdventureState, slot: int = 0) -> void:
    """保存冒险状态"""
    var save_path = "user://saves/adventure_%s/save_%d.json" % [state.adventure_id, slot]
    var save_data = {
        "adventure_state": state.to_dict(),
        "party_snapshot": _get_party_snapshot(),
        "world_state_snapshot": WorldStateManager.get_snapshot(),
        "timestamp": Time.get_unix_time_from_system()
    }
    _write_json(save_path, save_data)

func load(adventure_id: String, slot: int = 0) -> AdventureState:
    """加载冒险状态"""
    var save_path = "user://saves/adventure_%s/save_%d.json" % [adventure_id, slot]
    var save_data = _read_json(save_path)
    return AdventureState.from_dict(save_data.adventure_state)
```

### 14.2 Signal定义

```gdscript
# scripts/signals/adventure_signals.gd
extends Node
...
# 选择事件
signal choice_presented(node_id: String, choices: Array)
signal choice_made(choice_id: String, outcome: String)
signal branch_selected(branch_id: String, target_node: String)

# 遭遇事件
signal encounter_triggered(encounter_id: String, enemies: Array)
signal encounter_completed(encounter_id: String, outcome: String)
signal boss_phase_changed(boss_id: String, phase: int)

# 战利品事件
signal loot_dropped(items: Array)
signal loot_collected(item_id: String, character_id: String)

# 结算事件
signal settlement_started(outcome: String)
signal xp_awarded(character_id: String, amount: int)
signal scar_applied(character_id: String, scar_id: String)
signal world_state_changed(changes: Array)
signal settlement_completed(adventure_id: String)

# 实例化进度
signal instantiation_progress(step: int, total: int, description: String)
```
...
├── scripts/
│   ├── adventure/
│   │   ├── adventure_manager.gd              # 主控制器 (Autoload)
│   │   ├── blueprint_parser.gd               # 蓝图解析器
│   │   ├── business_validator.gd             # 业务逻辑验证
│   │   ├── adventure_instantiator.gd         # 实例化引擎
│   │   ├── adventure_state_manager.gd        # 状态管理器
│   │   │
│   │   ├── encounter/
│   │   │   ├── encounter_generator.gd        # 遭遇生成器
│   │   │   ├── cr_budget_calculator.gd       # CR预算计算
│   │   │   ├── enemy_composer.gd             # 敌人组合生成
│   │   │   ├── formation_placer.gd           # 阵型放置
│   │   │   └── boss_generator.gd             # Boss数据生成
│   │   │
│   │   ├── loot/
│   │   │   ├── loot_generator.gd             # 战利品生成器
│   │   │   ├── loot_distributor.gd           # 战利品分配
│   │   │   └── merchant_generator.gd         # 商人库存生成
│   │   │
│   │   ├── branch/
│   │   │   ├── choice_resolver.gd            # 选择解析器
│   │   │   ├── choice_tracker.gd             # 选择追踪器
│   │   │   └── condition_checker.gd          # 条件检查器
│   │   │
│   │   ├── settlement/
│   │   │   ├── settlement_system.gd          # 结算系统
│   │   │   ├── xp_calculator.gd              # XP计算
│   │   │   ├── scar_applier.gd               # 伤疤应用
│   │   │   └── world_state_mutator.gd        # 世界状态变更
│   │   │
│   │   ├── templates/
│   │   │   ├── template_selector.gd          # 模板选择器
│   │   │   └── template_adjuster.gd          # 模板难度调整
│   │   │
│   │   └── difficulty/
│   │       ├── difficulty_scaler.gd          # 难度缩放器
│   │       └── dynamic_adjuster.gd           # 动态难度调整
│   │
│   └── signals/
│       └── adventure_signals.gd              # 冒险信号总线
│
├── data/
│   ├── adventure_templates/
│   │   ├── template_short_01.json            # 离线短冒险模板
│   │   ├── template_short_02.json
│   │   ├── ...
│   │   ├── template_medium_01.json           # 离线中冒险模板
│   │   ├── ...
│   │   ├── template_long_01.json             # 离线长冒险模板
│   │   └── ...
│   │
│   ├── enemy_templates/
│   │   ├── goblin.json
...
│   │   ├── kobold.json
│   │   ├── goblin_boss.json
│   │   ├── skeleton_champion.json
│   │   ├── bandit_leader.json
│   │   └── troll.json
│   │
│   ├── loot_tables/
│   │   ├── tier_1_common.json
│   │   ├── tier_2_uncommon.json
│   │   ├── tier_3_rare.json
│   │   ├── boss_tier_legendary.json
│   │   └── ...
│   │
│   ├── room_templates/
│   │   ├── combat_arena.json
│   │   ├── combat_ambush.json
│   │   ├── combat_corridor.json
│   │   ├── dialogue_chamber.json
│   │   ├── merchant_camp.json
│   │   ├── rest_grotto.json
│   │   └── boss_throne.json
│   │
...
│       ├── encounter_data.schema.json
│       └── adventure_state.schema.json
│
├── scenes/
│   └── adventure/
│       ├── adventure_scene.tscn              # 冒险主场景
│       ├── settlement_screen.tscn            # 结算界面
│       └── choice_dialog.tscn                # 选择对话框
│
└── tests/
    └── adventure/
        ├── test_blueprint_parser.gd
        ├── test_business_validator.gd
        ├── test_encounter_generator.gd
        ├── test_cr_budget_calculator.gd
        ├── test_loot_generator.gd
        ├── test_choice_resolver.gd
        ├── test_adventure_state.gd
        └── test_settlement.gd
```
