...
```
res://scenes/combat/
├── combat_scene.tscn              # 战斗主场景
│   ├── CombatManager (Node)       # 战斗管理器
│   ├── TurnManager (Node)         # 回合管理器
│   ├── ActionResolver (Node)      # 行动解析器
│   ├── SpellResolver (Node)       # 法术解析器
│   ├── AIBehavior (Node)          # AI行为系统
│   ├── InitiativeTracker (Node)   # 先攻追踪器
│   ├── ConditionManager (Node)    # 条件管理器
│   ├── TerrainManager (Node)      # 地形管理器
│   ├── CombatUI (CanvasLayer)     # 战斗UI层
│   │   ├── InitiativeBar          # 先攻条
│   │   ├── ActionMenu             # 行动菜单
│   │   ├── CharacterStatus        # 角色状态栏
│   │   ├── CombatLog              # 战斗日志
│   │   └── DiceRoller             # 骰子动画
│   └── Battlefield (TileMap)      # 战场地图
│       ├── GridOverlay            # 网格覆盖层
│       ├── EffectOverlay          # 效果覆盖层
│       └── CharacterSprites       # 角色精灵层
```
...
var current_participant_index: int = -1
var current_round: int = 0
var current_state: CombatState = CombatState.INITIALIZATION
...
## 初始化战斗
func initialize_combat(player_party: Array, enemy_party: Array, terrain_data: Dictionary):
    current_state = CombatState.INITIALIZATION
    # 加载参与者
    for player in player_party:
        participants.append(CombatParticipant.new(player, true))
    for enemy in enemy_party:
        participants.append(CombatParticipant.new(enemy, false))
    # 加载地形
    # 初始化UI
    combat_started.emit()
    transition_to(CombatState.ROLL_INITIATIVE)

## 状态转换
func transition_to(new_state: CombatState):
    var old_state = current_state
    if not can_transition(old_state, new_state):
        push_error("Invalid transition: %s -> %s" % [old_state, new_state])
        return
...
## 执行状态逻辑
func execute_state(state: CombatState):
    match state:
        CombatState.ROLL_INITIATIVE:
            roll_initiative()
        CombatState.ROUND_START:
            process_round_start()
        CombatState.TURN_START:
            start_turn()
        CombatState.ACTION_PHASE:
            enter_action_phase()
        CombatState.BONUS_ACTION_PHASE:
            enter_bonus_action_phase()
        CombatState.MOVEMENT_PHASE:
            enter_movement_phase()
        CombatState.REACTION_WINDOW:
            enter_reaction_window()
        CombatState.TURN_END:
            end_turn()
        CombatState.ROUND_END:
            end_round()
        CombatState.VICTORY:
            process_victory()
        CombatState.DEFEAT:
...
func roll_initiative():
    ...
    for participant in participants:
        if participant.is_alive():
            ...
            var dex_mod = participant.character.get_ability_modifier("dex")
            var total = roll + dex_mod + participant.initiative_bonus
            participant.initiative_roll = roll
            participant.initiative_total = total
            results.append({"participant": participant, "roll": roll, "total": total})
    # 排序
    participants.sort_custom(func(a, b): return a.initiative_total > b.initiative_total)
    initiative_rolled.emit(results)
    transition_to(CombatState.ROUND_START)
```

#### TurnManager

```gdscript
# res://scripts/combat/turn_manager.gd
class_name TurnManager
extends Node

signal turn_order_changed()
signal current_combatant_changed(participant: CombatParticipant)

var turn_order: Array[CombatParticipant] = []
var current_index: int = -1

func set_turn_order(participants: Array[CombatParticipant]):
    turn_order = participants.duplicate()
    current_index = -1
    turn_order_changed.emit()

func get_next_combatant() -> CombatParticipant:
    current_index += 1
    if current_index >= turn_order.size():
        return null  # 轮结束
    var combatant = turn_order[current_index]
    current_combatant_changed.emit(combatant)
    return combatant

func is_round_complete() -> bool:
    return current_index >= turn_order.size()
...
```gdscript
# res://scripts/combat/action_resolver.gd
class_name ActionResolver
extends Node

signal action_resolved(result: Dictionary)
signal damage_dealt(target_id: String, damage: int, damage_type: String)
signal attack_missed(target_id: String)

## 解析攻击
func resolve_attack(attacker: CombatParticipant, target: CombatParticipant, 
                    weapon: ItemBaseResource, is_off_hand: bool = false) -> Dictionary:
    # 计算攻击加值
    var attack_bonus = calculate_attack_bonus(attacker, weapon)
    
    # 骰攻击检定
    var attack_roll = DiceRoller.roll_d20()
    var is_critical = attack_roll == 20
    var is_fumble = attack_roll == 1
    var attack_total = attack_roll + attack_bonus
    
    # 检查命中
    var target_ac = target.get_ac()
    var is_hit = is_critical or (not is_fumble and attack_total >= target_ac)
    
    if not is_hit:
        attack_missed.emit(target.character_id)
        return {"hit": false, "critical": false, "fumble": is_fumble}
    
    # 计算伤害
    var damage_result = calculate_damage(attacker, weapon, is_critical, is_off_hand)
    
    # 应用伤害
    apply_damage(target, damage_result)
...
## 计算攻击加值
func calculate_attack_bonus(attacker: CombatParticipant, weapon: ItemBaseResource) -> int:
    var bonus = 0
    # 属性调整值
    if weapon.weapon_data.properties.has(WeaponData.WeaponProperty.FINESSE):
        bonus += max(attacker.character.get_ability_modifier("str"),
                     attacker.character.get_ability_modifier("dex"))
    elif weapon.weapon_data.weapon_range == WeaponData.WeaponRange.RANGED:
        bonus += attacker.character.get_ability_modifier("dex")
    else:
        bonus += attacker.character.get_ability_modifier("str")
    
    # 熟练加值
    if attacker.character.has_weapon_proficiency(weapon):
        bonus += attacker.character.proficiency_bonus
    
    # 魔法加值
    if weapon.magical_data:
        for enchant in weapon.magical_data.enchantments:
            if enchant.effect.has("stat_modifiers") and enchant.effect.stat_modifiers.has("attack_roll"):
                bonus += enchant.effect.stat_modifiers.attack_roll
    
    return bonus

## 计算伤害
func calculate_damage(attacker: CombatParticipant, weapon: ItemBaseResource, 
                      is_critical: bool, is_off_hand: bool) -> Dictionary:
    var weapon_data = weapon.weapon_data
    var dice_str = weapon_data.damage_dice
    ...
    # 属性调整值
    var ability_mod = 0
    if weapon_data.properties.has(WeaponData.WeaponProperty.FINESSE):
        ability_mod = max(attacker.character.get_ability_modifier("str"),
                         attacker.character.get_ability_modifier("dex"))
    elif weapon_data.weapon_range == WeaponData.WeaponRange.RANGED:
        ability_mod = attacker.character.get_ability_modifier("dex")
    else:
        ability_mod = attacker.character.get_ability_modifier("str")
    
    # 副手: 不加属性调整值到伤害（除非有双持战斗风格）
    if is_off_hand and not attacker.character.has_fighting_style("two_weapon_fighting"):
        ability_mod = 0
    
    # 附魔伤害
    var bonus_damage = 0
    var bonus_dice_total = 0
    if weapon.magical_data:
        for enchant in weapon.magical_data.enchantments:
            if enchant.effect.has("stat_modifiers") and enchant.effect.stat_modifiers.has("damage_roll"):
                bonus_damage += enchant.effect.stat_modifiers.damage_roll
            if enchant.effect.has("damage_bonus"):
                var bonus_dice = enchant.effect.damage_bonus.dice
                if is_critical:
                    bonus_dice_total += DiceRoller.max_roll(bonus_dice)
                else:
                    bonus_dice_total += DiceRoller.roll(bonus_dice)
    
    var total = base_roll + ability_mod + bonus_damage + bonus_dice_total
    ...
    return {
        ...
        "bonus_dice": bonus_dice_total,
        "damage_type": weapon_data.damage_type
    }

## 应用伤害
func apply_damage(target: CombatParticipant, damage_result: Dictionary):
    var damage = damage_result.total
    var damage_type = damage_result.damage_type
    
    # 应用抗性/免疫/易伤
    damage = target.apply_resistance(damage, damage_type)
    
    # 先扣临时HP
    if target.character.hp_temporary > 0:
        var temp_absorbed = min(target.character.hp_temporary, damage)
        target.character.hp_temporary -= temp_absorbed
        damage -= temp_absorbed
    
    # 扣真实HP
    var old_hp = target.character.hp_current
    target.character.hp_current = max(0, target.character.hp_current - damage)
    
    damage_dealt.emit(target.character_id, damage, damage_type)
    
    # 检查是否死亡
    if target.character.hp_current == 0:
        target.on_knocked_unconscious()
```

#### SpellResolver

```gdscript
# res://scripts/combat/spell_resolver.gd
class_name SpellResolver
extends Node

signal spell_cast(caster_id: String, spell_id: String, targets: Array)
signal spell_failed(caster_id: String, reason: String)
signal concentration_check_required(caster_id: String, damage: int)
signal concentration_broken(caster_id: String)

## 施放法术
func cast_spell(caster: CombatParticipant, spell_data: Dictionary, 
                targets: Array, slot_level: int = -1) -> Dictionary:
    # 检查法术位
    if not caster.character.has_spell_slot(spell_data.level, slot_level):
        spell_failed.emit(caster.character_id, "no_spell_slots")
        return {"success": false, "reason": "no_spell_slots"}
    
    # 消耗法术位
    var used_level = slot_level if slot_level > 0 else spell_data.level
    caster.character.consume_spell_slot(used_level)
    ...
    # 解析法术效果
    var result = resolve_spell_effect(caster, spell_data, targets, used_level)
    
    # 标记专注
    if spell_data.get("concentration", false):
        caster.start_concentration(spell_data, result.get("duration", 0))
    
    spell_cast.emit(caster.character_id, spell_data.id, targets)
    return result

## 解析法术效果
func resolve_spell_effect(caster: CombatParticipant, spell_data: Dictionary, 
                          targets: Array, slot_level: int) -> Dictionary:
    var effect_type = spell_data.get("effect_type", "damage")
    
    match effect_type:
        "damage":
            return resolve_damage_spell(caster, spell_data, targets, slot_level)
        "healing":
            return resolve_healing_spell(caster, spell_data, targets, slot_level)
        "condition":
            return resolve_condition_spell(caster, spell_data, targets, slot_level)
        "utility":
            return resolve_utility_spell(caster, spell_data, targets, slot_level)
        _:
            return {"success": false}

## 解析伤害法术
func resolve_damage_spell(caster: CombatParticipant, spell_data: Dictionary, 
                          targets: Array, slot_level: int) -> Dictionary:
    var results = []
    
    for target in targets:
        # 法术攻击或豁免
        if spell_data.get("requires_attack_roll", false):
            # 法术攻击检定
            var attack_roll = DiceRoller.roll_d20()
            var spell_attack_mod = caster.character.spell_attack_modifier
            var is_critical = attack_roll == 20
            var attack_total = attack_roll + spell_attack_mod
            
            if is_critical or attack_total >= target.get_ac():
                var damage = calculate_spell_damage(spell_data, slot_level, is_critical)
                apply_damage_to_target(target, damage, spell_data.damage_type)
                results.append({"target": target.character_id, "hit": true, "damage": damage})
            else:
                results.append({"target": target.character_id, "hit": false})
        
        elif spell_data.get("save_required", false):
            # 豁免检定
            var save_ability = spell_data.save_ability
            var save_dc = caster.character.spell_save_dc
            var save_roll = DiceRoller.roll_d20()
            var save_mod = target.character.get_saving_throw_modifier(save_ability)
            var save_total = save_roll + save_mod
            
            var damage = calculate_spell_damage(spell_data, slot_level, false)
            
            if save_total >= save_dc:
                # 豁免成功
                if spell_data.get("save_effect", "half") == "half":
                    damage = damage / 2
                else:
                    damage = 0
            # 豁免失败: 全伤
...
## 计算法术伤害
func calculate_spell_damage(spell_data: Dictionary, slot_level: int, 
                            is_critical: bool) -> int:
    var base_dice = spell_data.get("damage_dice", "1d6")
    var bonus_per_level = spell_data.get("bonus_dice_per_slot_level", 0)
    
    # 升环加成
    var extra_dice = 0
    if slot_level > spell_data.level:
        extra_dice = (slot_level - spell_data.level) * bonus_per_level
    
    var total_dice = base_dice
    if extra_dice > 0:
        total_dice = add_dice(base_dice, extra_dice)
    
    # 暴击最大化
    if is_critical:
        return DiceRoller.max_roll(total_dice)
    else:
        return DiceRoller.roll(total_dice)

## 专注检定
func check_concentration(participant: CombatParticipant, damage: int) -> bool:
    var dc = max(10, damage / 2)
    var con_mod = participant.character.get_saving_throw_modifier("con")
    var roll = DiceRoller.roll_d20()
    var total = roll + con_mod
    
    if total < dc:
        participant.break_concentration()
        concentration_broken.emit(participant.character_id)
        return false
    return true
```
...
```gdscript
# res://scripts/combat/ai_behavior.gd
class_name AIBehavior
extends Node

signal ai_action_decided(participant: CombatParticipant, action: Dictionary)

## AI决策
func decide_action(participant: CombatParticipant, 
                   allies: Array[CombatParticipant], 
                   enemies: Array[CombatParticipant]) -> Dictionary:
    var enemy_type = participant.enemy_data.get("behavior", {}).get("type", "basic_melee")
    
    match enemy_type:
        "skirmisher":
            return decide_skirmisher_action(participant, allies, enemies)
        "undead_soldier":
            return decide_undead_action(participant, allies, enemies)
        "basic_melee":
            return decide_basic_melee_action(participant, allies, enemies)
        "berserker":
            return decide_berserker_action(participant, allies, enemies)
        _:
            return decide_basic_melee_action(participant, allies, enemies)

## 选择目标
func select_target(participant: CombatParticipant, 
                   enemies: Array[CombatParticipant]) -> CombatParticipant:
    ...
    for enemy in enemies:
        ...
        var score = 0
        var distance = get_distance(participant, enemy)
        
        # 距离因素
        score += max(0, 20 - distance)
        
        # HP因素
        var hp_ratio = float(enemy.character.hp_current) / float(enemy.character.hp_max)
        score += (1.0 - hp_ratio) * 15
        
        # 威胁因素
        score += estimate_threat(enemy) * 2
        
        # 专注因素
        if enemy.is_concentrating():
            score += 10
...
## 基础近战行为
func decide_basic_melee_action(participant: CombatParticipant, 
                               allies: Array, enemies: Array) -> Dictionary:
    var target = select_target(participant, enemies)
    if target == null:
        return {"action": "wait"}
    
    var distance = get_distance(participant, target)
    ...
    if distance <= 5:
        ...
        return {
            "action": "attack",
            "target": target,
            "weapon": participant.get_main_weapon()
        }
    else:
        # 移动然后攻击
        return {
            ...
            "target": target,
            "move_to": get_closest_reachable_position(participant, target),
            "weapon": participant.get_main_weapon()
        }
```
...
```gdscript
# res://scripts/combat/combat_signals.gd
extends Node

## 战斗生命周期
signal combat_started(encounter_id: String)
signal combat_ended(victory: bool, encounter_id: String)
signal round_started(round_number: int)
...
## 行动执行
signal action_used(combatant_id: String, action_type: String)
signal bonus_action_used(combatant_id: String, action_type: String)
signal movement_used(combatant_id: String, distance: int)
signal reaction_used(combatant_id: String, reaction_type: String)

## 攻击与伤害
signal attack_roll(attacker_id: String, target_id: String, roll: int, total: int, hit: bool, critical: bool)
signal damage_dealt(target_id: String, damage: int, damage_type: String, source_id: String)
signal healing_received(target_id: String, amount: int, source_id: String)

## 法术
signal spell_cast(caster_id: String, spell_id: String, slot_level: int, targets: Array)
signal spell_failed(caster_id: String, spell_id: String, reason: String)
signal concentration_check(caster_id: String, dc: int, roll: int, success: bool)
signal concentration_broken(caster_id: String)

## 条件
signal condition_applied(target_id: String, condition_type: String, duration: int, source_id: String)
signal condition_removed(target_id: String, condition_type: String)
signal condition_tick(target_id: String, condition_type: String, remaining_rounds: int)

## 状态变化
signal hp_changed(combatant_id: String, old_hp: int, new_hp: int, max_hp: int)
signal combatant_died(combatant_id: String, cause: String)
signal combatant_unconscious(combatant_id: String)
signal combatant_stabilized(combatant_id: String)
signal death_save_progressed(combatant_id: String, rounds_without_healing: int)

## 环境
signal terrain_effect_triggered(effect_id: String, affected_ids: Array)
signal object_interacted(object_id: String, interaction_type: String, combatant_id: String)
```
...
class CombatParticipant:
    ...
    var is_player: bool
    var enemy_data: Dictionary  # 仅敌人使用
    
    # 战斗状态
    var initiative_roll: int = 0
    var initiative_total: int = 0
    ...
    # 专注
    var concentrating_on: Dictionary = {}  # 法术数据
    var concentration_duration: int = 0
    ...
    func is_concentrating() -> bool:
        return not concentrating_on.is_empty()
    
    func get_ac() -> int:
        return character.armor_class
    
    func start_concentration(spell_data: Dictionary, duration: int):
        concentrating_on = spell_data
        concentration_duration = duration
    
    func break_concentration():
        concentrating_on = {}
        concentration_duration = 0
    
    func on_knocked_unconscious():
        character.death_save_rounds_without_healing = 0
        break_concentration()
        # 清除行动资源
...
│   │   ├── action_resolver.gd         # 行动解析器
│   │   ├── spell_resolver.gd          # 法术解析器
│   │   ├── ai_behavior.gd             # AI行为系统
│   │   ├── initiative_tracker.gd      # 先攻追踪器
│   │   ├── condition_manager.gd       # 条件管理器
│   │   ├── terrain_manager.gd         # 地形管理器
│   │   ├── damage_calculator.gd       # 伤害计算器
│   │   ├── attack_resolver.gd         # 攻击解析器
│   │   ├── save_resolver.gd           # 豁免解析器
│   │   ├── rest_manager.gd            # 短休/长休管理器
│   │   └── combat_participant.gd      # 战斗参与者类
│   ├── signals/
│   │   └── combat_signals.gd          # 战斗信号总线
│   └── data/
│       ├── enemies/                    # 敌人数据
│       │   ├── goblin.json
│       │   ├── skeleton.json
...
│       ├── combat_scene.tscn           # 战斗主场景
│       ├── combat_ui.tscn              # 战斗UI
│       ├── initiative_bar.tscn         # 先攻条
│       ├── action_menu.tscn            # 行动菜单
│       ├── dice_roller.tscn            # 骰子动画
│       └── damage_popup.tscn           # 伤害弹出
└── tests/
    └── combat/
        ├── test_attack_roll.gd
        ├── test_damage_calculation.gd
        ├── test_spell_resolution.gd
