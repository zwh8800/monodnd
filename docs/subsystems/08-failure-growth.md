...
角色 A (战士, 存活, 最高伤害):
  ...
  速度: 正常 (×1.0)
  冒险完成: +300
  总 XP: (1500 × 1.1 × 1.1) + 300 = 1,815 + 300 = 2,115
...
角色 C (盗贼, 最终战倒地被救回):
  参与度: 0.9
  总 XP: (1500 × 0.9 × 1.1) + 300 = 1,485 + 300 = 1,785
...
### 13.1 SettlementManager 类

```gdscript
# res://scripts/settlement/settlement_manager.gd
class_name SettlementManager
extends Node

## 结算管理器 - 处理冒险成功和失败的完整流程

signal settlement_started(adventure_id: String, is_success: bool)
signal settlement_completed(adventure_id: String, results: Dictionary)
signal xp_distributed(character_id: String, amount: int)
signal penalties_applied(penalties: Array[Dictionary])
signal narrative_generated(narrative: String)

# 依赖注入
@export var character_manager: CharacterManager
@export var world_state_manager: WorldStateManager
@export var reputation_manager: ReputationManager
@export var economy_manager: EconomyManager
@export var inheritance_manager: InheritanceManager
@export var llm_gateway: LLMGateway

var current_adventure: Dictionary = {}
var settlement_results: Dictionary = {}

func start_settlement(adventure_id: String, is_success: bool) -> void:
    current_adventure = AdventureDatabase.get_adventure(adventure_id)
    settlement_results = {}
    
    emit_signal("settlement_started", adventure_id, is_success)
    
    if is_success:
        await _process_success_settlement()
    else:
        await _process_failure_settlement()
    
    emit_signal("settlement_completed", adventure_id, settlement_results)

func _process_success_settlement() -> void:
    # Step 1: XP 计算与分配
    var xp_results = await _calculate_and_distribute_xp()
    settlement_results["xp"] = xp_results
    
    # Step 2: 战利品生成
    var loot_results = await _generate_loot()
    settlement_results["loot"] = loot_results
    
    # Step 3: 关系更新
    var relationship_results = _update_relationships()
    settlement_results["relationships"] = relationship_results
    
    # Step 4: 声望提升
    var reputation_gain = _calculate_reputation_gain(true)
    reputation_manager.add_reputation(reputation_gain)
    settlement_results["reputation_change"] = reputation_gain
    
    # Step 5: 世界状态正向变化
    var world_changes = _apply_world_state_changes(true)
    settlement_results["world_changes"] = world_changes
    
    # Step 6: 成就记录
    _record_achievements()
    
    # Step 7: LLM 胜利叙事
    var narrative = await _generate_success_narrative()
    settlement_results["narrative"] = narrative

func _process_failure_settlement() -> void:
    # Step 1: 确定失败严重程度
    var severity = _determine_failure_severity()
    settlement_results["severity"] = severity
    
    # Step 2: 生成惩罚
    var penalties = await _generate_penalties(severity)
    settlement_results["penalties"] = penalties
    
    # Step 3: 应用角色后果
    var character_consequences = _apply_character_consequences(severity)
    settlement_results["character_consequences"] = character_consequences
    
    # Step 4: 应用装备后果
    var equipment_consequences = _apply_equipment_consequences(severity)
    settlement_results["equipment_consequences"] = equipment_consequences
    
    # Step 5: 世界状态负向变化
    var world_changes = _apply_world_state_changes(false)
    settlement_results["world_changes"] = world_changes
    
    # Step 6: 声望损失
    var reputation_loss = _calculate_reputation_gain(false)
    reputation_manager.add_reputation(reputation_loss)
    settlement_results["reputation_change"] = reputation_loss
    
    # Step 7: LLM 失败叙事
    var narrative = await _generate_failure_narrative(severity)
    settlement_results["narrative"] = narrative
    
    # Step 8: 如果有角色死亡，触发传承
    if character_consequences.has("deaths"):
        for death in character_consequences["deaths"]:
            inheritance_manager.trigger_inheritance(death.character_id)

func _determine_failure_severity() -> String:
    var party_state = _get_party_state()
    var adventure_progress = _get_adventure_progress()
    
    # 灾难性: 全灭
    if party_state.dead_count == party_state.total_members:
        return "catastrophic"
    
    # 严重: 有角色死亡
    if party_state.dead_count >= 1:
        return "severe"
    
    # 中等: 大部分倒地或完成度低
    if party_state.unconscious_count >= party_state.total_members * 0.5:
        return "moderate"
    if adventure_progress.completion_percentage < 0.75:
        return "moderate"
    
    # 轻微: 其他情况
    return "minor"

func _calculate_and_distribute_xp() -> Dictionary:
    var results = {}
    var encounters = current_adventure.encounters
    var party = character_manager.get_active_party()
    var avg_level = _calculate_average_level(party)
    
    for character in party:
        var xp = _calculate_xp_for_character(character, encounters, avg_level)
        character.add_xp(xp)
        results[character.character_id] = xp
        emit_signal("xp_distributed", character.character_id, xp)
...
```gdscript
# res://scripts/settlement/scar_manager.gd
class_name ScarManager
extends Node

## 伤疤管理器 - 处理伤疤生成、应用和移除

signal scar_acquired(character_id: String, scar_data: Dictionary)
signal scar_removed(character_id: String, scar_id: String)

@export var scar_pool: Array[Dictionary] = []  # 从 JSON 加载
@export var llm_gateway: LLMGateway

func generate_scar(
    character: CharacterResource,
    combat_log: Array[Dictionary],
    damage_summary: Dictionary,
    failure_severity: String
) -> Dictionary:
    # Step 1: 确定主要伤害类型
    var primary_damage_type = _get_primary_damage_type(damage_summary)
    
    # Step 2: 确定伤疤严重程度
    var scar_severity = _determine_scar_severity(failure_severity)
    ...
    # Step 4: 加权随机选择
    var selected_scar = _weighted_random_select(candidates, scar_severity)
    
    # Step 5: 生成 LLM 叙事
    var narrative = await _generate_scar_narrative(
        character, selected_scar, combat_log
    )
    
    # Step 6: 构建完整伤疤数据
    var scar_data = {
        "scar_id": _generate_unique_id(),
        "character_id": character.character_id,
        "acquired_at": {
            "adventure_id": current_adventure.id,
    ...
    # Step 7: 应用到角色
    character.scar_ids.append(scar_data.scar_id)
    _apply_mechanical_effects(character, scar_data.mechanical_effects)
    
    emit_signal("scar_acquired", character.character_id, scar_data)
    return scar_data

func _determine_scar_severity(failure_severity: String) -> String:
    match failure_severity:
        "minor":
            return "light"
        "moderate":
            return _weighted_random({"light": 60, "moderate": 35, "severe": 5})
        "severe":
            return _weighted_random({"light": 30, "moderate": 50, "severe": 20})
        "catastrophic":
            return _weighted_random({"light": 10, "moderate": 40, "severe": 50})
        _:
            return "light"
...
func _filter_candidates(
    damage_type: String,
    severity: String,
    existing_scar_ids: Array[String]
) -> Array[Dictionary]:
    var candidates = []
    for scar in scar_pool:
        if scar.damage_type == damage_type:
            if scar.id not in existing_scar_ids:
                candidates.append(scar)
    return candidates

func remove_scar(character: CharacterResource, scar_id: String, method: String) -> bool:
    # 验证移除条件
    if not _can_remove_scar(character, scar_id, method):
        return false
    
    # 移除伤疤
    character.scar_ids.erase(scar_id)
    _remove_mechanical_effects(character, scar_id)
    
    emit_signal("scar_removed", character.character_id, scar_id)
    return true
```

### 13.3 WorldStateManager 类

```gdscript
# res://scripts/world/world_state_manager.gd
...
signal world_state_mutated(changes: Array[Dictionary])
...
func load_world_state() -> void:
    if FileAccess.file_exists(SAVE_PATH):
        ...
        if error == OK:
            world_state = json.data
            emit_signal("world_state_loaded")
        else:
...
func apply_mutations(changes: Array[Dictionary]) -> void:
    for change in changes:
        match change.type:
            "region_state":
                _apply_region_change(change)
            "faction_disposition":
                _apply_faction_change(change)
            "npc_status":
                _apply_npc_change(change)
            "threat_level":
                _apply_threat_change(change)
    
    save_world_state()
    emit_signal("world_state_mutated", changes)

func get_context_for_adventure_generation() -> Dictionary:
    return {
        "regions": world_state.regions,
        "factions": world_state.factions,
        "npcs": world_state.npcs,
        "adventure_history": world_state.adventure_history
    }
```

### 13.4 ReputationManager 类

```gdscript
# res://scripts/tavern/reputation_manager.gd
...
signal reputation_threshold_crossed(new_tier: String)

@export var tavern_level_manager: TavernLevelManager
...
func add_reputation(amount: int) -> void:
    ...
    reputation = clampi(reputation + amount, MIN_REPUTATION, MAX_REPUTATION)
    
    emit_signal("reputation_changed", old_reputation, reputation)
    
    var old_tier = _get_reputation_tier(old_reputation)
    var new_tier = _get_reputation_tier(reputation)
    
    if old_tier != new_tier:
        emit_signal("reputation_threshold_crossed", new_tier)
...
func get_reputation_multiplier() -> float:
    match _get_reputation_tier(reputation):
        "novice": return 0.8
        "known": return 1.0
        "respected": return 1.1
        "famous": return 1.2
        "legendary": return 1.3
        _: return 1.0
```

### 13.5 InheritanceManager 类

```gdscript
# res://scripts/inheritance/inheritance_manager.gd
class_name InheritanceManager
extends Node

## 继承管理器 - 处理角色死亡/退役后的知识传承

signal inheritance_triggered(from_id: String, inheritance_data: Dictionary)
signal heritage_points_spent(amount: int, effect: String)

@export var character_manager: CharacterManager
@export var llm_gateway: LLMGateway

var pending_inheritances: Array[Dictionary] = []

func trigger_inheritance(dead_character_id: String) -> void:
    var dead_character = character_manager.get_character(dead_character_id)
    
    var inheritance_data = {
        "from_character": {
            "character_id": dead_character.character_id,
            "name": dead_character.character_name,
            "class": dead_character.primary_class_id,
            "level": dead_character.level,
            "cause": "death_in_combat"
        },
        "inherited_skill": _select_inherited_skill(dead_character),
        "heritage_points": _calculate_heritage_points(dead_character),
        "knowledge_tags": _extract_knowledge_tags(dead_character),
        "inherited_equipment": _select_inherited_equipment(dead_character),
        "biography": await _generate_biography(dead_character)
    }
    
    pending_inheritances.append(inheritance_data)
    emit_signal("inheritance_triggered", dead_character_id, inheritance_data)

func _calculate_heritage_points(character: CharacterResource) -> Dictionary:
    var points_from_xp = floori(character.xp / 500)
    var points_from_level = _get_level_points(character.level)
    var total = points_from_xp + points_from_level
    ...
    return {
        ...
        "available": total,
        "calculation": {
            "from_character_level": character.level,
            "xp_converted": character.xp,
            "points_from_xp": points_from_xp,
            "points_from_level": points_from_level
        }
    }

func _get_level_points(level: int) -> int:
    if level <= 5:
        return 1
    elif level <= 10:
        return 2
...
func spend_heritage_points(
    new_character_id: String,
    amount: int,
    effect: String
) -> bool:
    var inheritance = _get_pending_inheritance(new_character_id)
    if inheritance == null:
        return false
    
    if inheritance.heritage_points.available < amount:
        return false
    
    inheritance.heritage_points.spent += amount
    inheritance.heritage_points.available -= amount
    
    emit_signal("heritage_points_spent", amount, effect)
    return true
```

### 13.6 EconomyManager 类

```gdscript
# res://scripts/economy/economy_manager.gd
class_name EconomyManager
extends Node

## 经济管理器 - 管理金币收支和经济平衡

signal gold_changed(old_amount: int, new_amount: int, reason: String)
...
func add_gold(amount: int, reason: String) -> void:
    var old_gold = party_gold
    party_gold = mini(party_gold + amount, MAX_GOLD)
    emit_signal("gold_changed", old_gold, party_gold, reason)
...
func calculate_sell_price(base_value: int) -> int:
    var reputation_mult = reputation_manager.get_reputation_multiplier()
    return int(base_value * 0.5 * reputation_mult)

func calculate_repair_cost(condition_level: int, item_level: int) -> int:
    var base_cost = 10 + (condition_level * 10)
    var level_mult = 1.0 + (item_level * 0.1)
    return int(base_cost * level_mult)

func apply_gold_loss_severity(severity: String) -> int:
    var loss_percentage = 0.0
    match severity:
        "minor":
            loss_percentage = 0.3
        "moderate":
    ...
    var loss = int(party_gold * loss_percentage)
    spend_gold(loss, "failure_penalty")
    return loss
```
...
```gdscript
# res://scripts/signals/settlement_signals.gd
extends Node

## 结算系统信号总线

# 冒险结算
signal adventure_completed(adventure_id: String, is_success: bool)
signal adventure_failed(adventure_id: String, cause: String)
signal settlement_started(adventure_id: String, is_success: bool)
signal settlement_completed(adventure_id: String, results: Dictionary)

# XP
signal xp_distributed(character_id: String, amount: int)
signal level_up_from_settlement(character_id: String, new_level: int)

# 惩罚
signal failure_severity_determined(severity: String)
signal penalties_applied(penalties: Array[Dictionary])
signal penalty_narrative_generated(penalty_id: String, narrative: String)

# 伤疤
signal scar_acquired(character_id: String, scar_data: Dictionary)
signal scar_removed(character_id: String, scar_id: String)
signal scar_narrative_generated(scar_id: String, narrative: String)

# 死亡
signal character_died(character_id: String, cause: String, adventure_id: String)
signal character_retired(character_id: String)
signal death_save_progressed(character_id: String, rounds: int)

# 装备
signal equipment_damaged(item_id: String, old_condition: String, new_condition: String)
signal equipment_destroyed(item_id: String, reason: String)
signal equipment_lost(item_id: String, reason: String)

# 世界状态
signal world_state_mutated(changes: Array[Dictionary])
signal region_state_changed(region_id: String, old_state: String, new_state: String)
signal faction_disposition_changed(faction_id: String, old_disp: String, new_disp: String)
signal npc_status_changed(npc_id: String, old_status: String, new_status: String)
...
signal inheritance_triggered(from_id: String, inheritance_data: Dictionary)
signal heritage_points_spent(amount: int, effect: String)
signal biography_generated(character_id: String, biography: String)
...
├── scripts/
│   ├── settlement/
│   │   ├── settlement_manager.gd          # 结算管理器
│   │   ├── scar_manager.gd                # 伤疤管理器
│   │   ├── death_manager.gd               # 死亡处理管理器
│   │   ├── inheritance_manager.gd         # 继承管理器
│   │   └── biography_generator.gd         # 传记生成器
│   ├── world/
│   │   ├── world_state_manager.gd         # 世界状态管理器 (单例)
│   │   └── world_state_mutation.gd        # 世界状态变异逻辑
│   ├── tavern/
│   │   ├── reputation_manager.gd          # 声望管理器
│   │   ├── tavern_level_manager.gd        # 酒馆等级管理器
│   │   └── economy_manager.gd             # 经济管理器
│   ├── signals/
│   │   └── settlement_signals.gd          # 结算系统信号总线
│   └── data/
│       ├── scars/
│       │   ├── fire_scars.json            # 火焰伤疤池
│       │   ├── cold_scars.json            # 寒冷伤疤池
│       │   ├── lightning_scars.json       # 闪电伤疤池
│       │   ├── necrotic_scars.json        # 黯蚀伤疤池
│       │   ├── psychic_scars.json         # 心灵伤疤池
│       │   ├── physical_scars.json        # 物理伤疤池
│       │   ├── poison_acid_scars.json     # 毒酸伤疤池
│       │   └── generic_scars.json         # 通用伤疤池
│       ├── knowledge_tags.json            # 知识标签定义
│       ├── heritage_point_costs.json      # 传承点消费表
│       └── settlement_templates.json      # 降级模板
├── scenes/
│   ├── settlement/
│   │   ├── success_settlement.tscn        # 成功结算界面
│   │   ├── failure_settlement.tscn        # 失败结算界面
│   │   ├── scar_acquisition.tscn          # 伤疤获得界面
│   │   ├── death_notification.tscn        # 死亡通知界面
│   │   └── inheritance_selection.tscn     # 继承选择界面
│   └── tavern/
│       ├── hero_wall.tscn                 # 英雄之壁界面
│       └── biography_viewer.tscn          # 传记查看界面
└── assets/
    ├── sprites/
    │   └── scars/                         # 伤疤视觉效果
    └── audio/
        ├── settlement_success.ogg         # 成功结算音效
        ├── settlement_failure.ogg         # 失败结算音效
        └── scar_acquisition.ogg           # 伤疤获得音效
```
