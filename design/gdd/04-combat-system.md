...
| 法术位恢复 | 短休部分恢复 | **短休恢复一半1环位（向上取整）** | 保留资源管理深度 | §7.1 |
| STR-DEX平衡 | STR仅近战伤害/负重 | **STR额外提供物理DR=STR_mod + 推撞擒抱优势** | 补偿每轮重骰先攻对DEX的放大；保留house rule的节奏特性 | 2026-05-09裁决 |
...
> 2026-05-09 `/review-all-gdds` 裁决：保留全部 DND 5e 规则深度，但将中级计算**被动化**以降低玩家认知负荷。

| 系统 | 处理方式 | UI 表达 |
|------|----------|---------|
| 伤害抗性/免疫/易伤 | 自动计算最终伤害（程序内部完成抗性÷2/免疫=0/易伤×2） | 显示最终伤害数字 + 抗性图标（🔥↘/❌/↗） |
| 专注检定 | 受到伤害时自动掷 d20+CON vs DC，无需玩家手动操作 | 检定失败时弹出"专注中断！"提示 |
| 优势/劣势汇总 | 多源优势+劣势自动合并为单一状态（标准化为优/劣/平骰） | 骰子动画旁显示 ⬆/⬇ 标识 |

**设计原则**: 所有 DND 规则仍然完整执行——只是计算过程对玩家透明。高级玩家可通过战斗日志查看详细判定链。这在保持战术深度的同时将活跃认知层从 >10 降至 6-7。

---

## 1A. 玩家体验幻想 (Player Fantasy)

> **本节是所有战斗系统设计决策的锚点。当任何机制与本节描述的感受冲突时，以本节为准。**

### 1A.1 核心情感承诺

当玩家进入一场战斗时，他们应该感受到：

- **"每一步决策都有重量"** — 先攻每轮重掷，行动按顺序选择并执行（非同时盲选）。玩家知道下一回合的顺序，能在充分信息下做出有意义的战术决策。移动是分段的（移动→行动→移动），每一步都值得斟酌。这不是"按最优Build自动运转"，而是"每一轮都在做有风险的选择"。

- **"规则是透明的"** — 每个d20结果的计算过程完全可见：攻击加值来源（STR_mod + PB + 魔法加值）、AC来源（护甲类型 + DEX上限 + 盾牌）、伤害来源（武器骰 + STR_mod + 附魔）。战斗日志记录每一轮的完整判定链。玩家从不猜测"为什么打不中"。

- **"死亡是真实的"** — 角色HP归零后进入3轮救援窗口。在这3轮中，队伍必须完成医疗检定或施放治疗——这创造了一种"我们正在和死神赛跑"的紧迫感。但如果在倒地状态下受到伤害，死亡会加速到来。双轨制让死亡既有紧迫感又保留了戏剧性的救援空间。

- **"DND的灵魂在这里"** — d20骰子落地的那一刻，d20物理根基保留。暴击（自然20）的伤害骰取最大值——看到那个金色的20，你知道自己打出了致命一击。状态条件（盲目/恐慌/麻痹等14种）让战斗不只是数字对撞——地精术士的恐慌光环让你的战士无法靠近，腐烂恶臭让你中毒后打什么都偏。

- **"我的构筑是有效的"** — 职业差异化不仅体现在伤害数值上。战士的额外攻击和动作潮涌让他在前线无可替代；法师的法术位管理决定了他何时释放火球、何时保留一线生机；盗贼的偷袭条件让玩家在战斗中不断寻找优势位置。这不是"哪个职业DPS最高"的问题，而是"这个局面谁最适合出手"。

### 1A.2 设计测试

当面对任何战斗设计决策时，问自己：

1. **信息测试**：在当前回合开始时，玩家能否根据可见信息（先攻顺序、位置、状态）做出有意义的决策？如果玩家在同一局面下总是做同样的选择，说明信息量不足或选择太少。
2. **紧张感测试**：一场战斗中，是否有至少一个时刻让玩家感到"这可能要输"？如果满血碾压贯穿始终，难度曲线有问题。
3. **死亡临近测试**：当一个角色HP=0倒地时，其他玩家角色的行动是否发生了可观察的变化（救援优先、路线改变、资源重新分配）？如果倒地了大家继续各打各的，说明死亡的重量感没有落地。
4. **构筑验证测试**：两个同职业不同Build的角色（如DEX战士 vs STR战士），在战斗中是否表现出可感知的差异化打法？
5. **节奏测试**：一场"普通难度"的遭遇（4个3级角色 vs CR 3）是否在3-5轮内结束？超过7轮说明节奏拖沓。

### 1A.3 感官锚点

- **视觉**：d20骰子的3D旋转动画（骰子声是DND的物理根基）、暴击时的金色闪光、倒地角色的红色濒死闪烁、状态条件图标在角色头像上的清晰展示
- **听觉**：骰子滚动→停止的音效（自然20有特殊音效）、武器打中护甲的金属撞击声（命中 vs 未命中有明显区分）、角色倒地时的沉重音效、暴击时的鼓点强化
- **触觉**：鼠标悬停在战斗日志上时高亮该动作在战场上的对应位置和数值计算链
...
| `ROLL_INITIATIVE` | 每轮开始（含首轮） | 每个参与者骰d20+DEX调整值；排序；处理突袭（仅首轮） | 所有参与者骰完 | `ROUND_START` |
...
| `RETREAT_CHECK` | 撤退投票通过 | 计算撤退DC；执行撤退检定（d20 + DEX_mod + 环境修正） | 检定完成 | `FLEE`（成功）或`ROUND_START`（失败，承受借机攻击后继续当前轮） |
| `FLEE` | 撤退检定成功 | 应用撤退惩罚；触发LLM撤退叙述；标记节点"已撤退" | 惩罚处理完成 | 返回探索阶段 |
| `VICTORY` | 所有敌人死亡/逃跑 | 触发战利品生成；触发LLM叙述；结算XP | 结算完成 | 返回探索阶段 |
...
public static class CombatTransitionGuards
{
    ...
    public static bool CanTransition(CombatState from, CombatState to, CombatContext context)
    {
        return (from, to) switch
        {
            ...
            (CombatState.RoundEnd, CombatState.RollInitiative)
            ...
            (CombatState.RetreatCheck, CombatState.RoundStart)
                => !context.RetreatCheckSucceeded(),  // 撤退失败→借机攻击→继续当前轮（不重骰先攻）
...
public class CombatContext
{
    ...
    public bool AllCombatantsActed()
        => CurrentCombatantIndex >= AllCombatants.Count - 1;
...
> **数据权威源**: 条件的定义、数据模型和效果以 `01-character-system.md` §2.8 为准。本节保留战斗中的触发/结束时机和堆叠规则——具体条件效果请交叉引用 character-system。
...
#### 11.3.1 行为类型统一映射 `[CUSTOM]`

> **2026-05-10 裁决**：敌方AI行为类型在多个文档中使用不同枚举值。以下为本系统（`04-combat-system.md`）与其他文档之间的统一映射表，以本条为权威源。

| 04/11 理论类型 | 04 §11.4 实现标识符 (`behavior.type`) | 06-adventure-generation `ai_behavior` | 说明 |
|:---:|------|------|------|
| 近战战士 | `basic_melee` | `aggressive_melee`, `melee_first` | 标准近战AI |
| 近战战士 | `berserker` | `berserker` | 狂战士变体（无撤退、优先最弱目标） |
| 近战战士 | `skirmisher` | `hit_and_run` | 游击变体（打完脱离） |
| 近战战士 | `undead_soldier` | `undead_guardian` | 亡灵变体（无自保、无畏） |
| 远程射手 | — (MVP未实现) | `ranged_kiter` | 远程保持距离射击 |
| 施法者 | `caster` | `caster` | 法术优先 |
| 治疗者 | — (MVP未实现) | `healer` | 治疗优先 |
| 刺客 | — (MVP未实现) | `assassin` | 潜行偷袭（Phase 2扩展） |
| 近战战士 (Boss) | — | `boss_commander`, `bandit_commander` | Boss专用变体（含阶段转换） |
| 施法者 (群战) | — | `pack_hunter`, `pack_swarm` | 群战协调变体 |

**实现指南**：
- 06-adventure-generation 在生成遭遇时使用 `ai_behavior` 字段
- 战斗初始化时，系统应根据上表将 `ai_behavior` 映射为标准行为类型
- 映射到同一理论类型的变体通过 `behavior` 对象中的 `special_tactics` 数组实现差异化
- 标记为"Phase 2扩展"的类型在MVP中回退到默认行为（近战战士）
...
> **结算管线权威源**: 2026-05-09 `/review-all-gdds` 裁决——`08-failure-growth.md` 是结算管线的**单一权威源**。战斗系统仅负责"战斗终止→信号发出"的过渡流程，所有 XP/战利品/关系/声望/世界状态/LLM 叙事等结算逻辑由 failure-growth 统一处理。
...
胜利过渡流程:
  1. 战斗动画结束
  2. 显示胜利画面
  3. 发出信号: combat_victory(encounter_id, participants)
  4. 控制权移交 adventure-generation → 触发 failure-growth 结算管线
  5. 战斗系统不再处理: XP分配、战利品生成、LLM叙事——全部委托 failure-growth
```
...
失败过渡流程:
  1. 战斗动画结束
  2. 显示失败画面
  3. 发出信号: combat_defeat(encounter_id, severity, participants)
  4. 控制权移交 failure-growth 进行统一失败结算
  5. 战斗系统不再处理: 惩罚应用、死亡处理——全部委托 failure-growth
```
...
> **⚠️ 非权威源**: 2026-05-09 裁决——失败惩罚的权威定义位于 `08-failure-growth.md` §3（失败结算流程）和 §4-5（伤疤/死亡处理）。以下为战斗侧参考摘要，如与 failure-growth 冲突，以 failure-growth 为准。
>
> 失败惩罚分为两大类：**战斗败退惩罚**（队伍被击败）和**主动撤退惩罚**（战略性撤退）。
...
| **中等** | 全队败退（被击败）但无人永久死亡 | 随机装备 condition降1级（Pristine→Worn 或 Worn→Damaged，详见 failure-growth §6.2） + 金币损失(10-20%) |
| **严重** | 有角色永久死亡（双轨死亡机制触发：3轮无治疗 或 死亡失败>=3） | 角色永久死亡 + 全队疲乏+1 + 装备 condition降级至 Broken（效果0%，需修理） |
| **灾难性** | 全灭（全员永久死亡） | 所有角色死亡 + 所有非绑定装备丢失（见 failure-growth §6.1 Catastrophic） + 金币损失50% |
...
| **装备损坏** | 1件随机装备 condition降1级（详见 failure-growth §6.2） | 匆忙中受损 |
...
装备损坏规则 [委托 failure-growth]:
  > 装备损坏的权威定义位于 `08-failure-growth.md` §6（装备耐久与修理）。
  > 本节仅描述战斗侧触发条件，具体 condition降级幅度和修理费用以 failure-growth 为准。
  > condition级别系统: Pristine(100%) → Worn(90%) → Damaged(75%) → Broken(0%) → Destroyed(丢失)
  >
  > condition降1级示例: Pristine→Worn (效果-10%) 或 Worn→Damaged (效果-15%)
  > condition降级至Broken: 装备效果降至0%，需修理
  > 全灭丢失: Catastrophic级别惩罚 → 非绑定装备 Destroyed（见 failure-growth §6.1）
...
```
XP分配规则:
  > ⚠️ 结算管线权威源: 2026-05-09 裁决——`08-failure-growth.md` §2 是 XP 计算的单一权威源。
  > 以下为战斗侧参考摘要，实际 XP 分配公式以 failure-growth 为准。
  - 每个击杀: 使用 failure-growth §2.2 的等级缩放公式（非标准 DND 5e CR→XP 查表）
  - 完成冒险: 基于冒险长度的完成奖励（见 failure-growth §2.2）
  - 多人满额获取: 每个角色独立获取全额 XP（不分摊）

  参考: DND 5e CR→XP 对照表（仅作设计参考，非实现引用）:
    CR 0: 10 XP
    CR 1/8: 25 XP
    CR 1/4: 50 XP
    CR 1/2: 100 XP
  ...
  注意: failure-growth 的 XP 公式为 Base XP = multiplier × avg_party_level，
  与上述标准 DND 5e CR→XP 表不同。以 failure-growth 为准。
...
投票规则:
  ...
  - 全员同意 → 立即进入撤退检定
  - 多数同意 + 领导者检定成功 → 进入撤退检定
    · 领导者检定: d20 + CHA(Persuasion) vs DC 12
    · 任一角色可担任提议者
...
撤退检定公式:
  ...
  环境修正（叠加到DC）:
    ...
    - 有明确出口: -2（有出口更容易逃脱）
    - 困难地形: +2（撤退路径有困难地形）
    - 已消灭半数以上敌人: -3（敌人士气低落，更容易逃脱）
    - Boss战: +5（Boss不会放走猎物）
    - 被包围（所有出口被敌人占据）: +5

  示例:
    普通遭遇(3哥布林), 已消灭1个, 有出口:
      DC = 10 + (+2存活×2) + (-2出口) + (-3已消灭半数) = 7
      检定: d20 + DEX_mod(最高) vs DC 7

    Boss战(巨魔), 无出口:
      DC = 18 + (+1存活) + (0无出口) + (+5 Boss) = 24
      检定: d20 + DEX_mod vs DC 24（极难逃脱——考虑引入替代逃生机制，如残血代价撤退）
```
...
TEST 43.6: 撤退检定DC计算
  - 普通遭遇(3哥布林), 已消灭1个, 有出口:
    DC = 10 + (+2存活×2) + (-2出口) + (-3消灭半数) = 7
  - Boss战(巨魔), 无出口:
    DC = 18 + (+1存活) + (0无出口) + (+5Boss) = 24
  - 被包围(所有出口被敌人占据), 3个存活敌人:
    DC = 10 + (+3存活×3) + (+5被包围) = 28 → clamp到最高DC 25

TEST 43.7: 撤退成功惩罚
  - 全队疲乏+1级
  - 金币损失10-20%
  - 1件随机装备 condition降1级（Pristine→Worn 或 Worn→Damaged）
  - 当前节点标记"已撤退"
...
> **骰子 Mock 策略**: 所有依赖 d20 的测试必须使用可种子化的 `IDiceRoller` 接口。测试实现通过 `MockDiceRoller(seed)` 注入预定骰子序列，确保每次运行结果完全一致。自然 1/20 通过种子控制，优势/劣势通过两个连续种子值验证。

| # | 验收标准 | 测试方法 | 通过条件 |
|---|----------|----------|----------|
| AC-1 | FSM 状态转换与中断机制正确 | 单元测试 | (a) 15 个定义状态间 17 条显式守卫（见 §2.4 `CombatTransitionGuards`）全部通过——每条守卫验证至少 1 正例+1 反例；(b) 5 种反应中断（OA/Shield/Counterspell/Protection/Uncanny Dodge）在正确时机触发并在正确恢复点继续原回合；(c) 无效转换被守卫拒绝后不产生未定义行为（无异常、无状态污染） |
| AC-2 | 顺序回合制按先攻执行，每轮重骰先攻 | 集成测试 | (a) 首轮按 ROLL_INITIATIVE 结果排序执行；(b) ROUND_END 后回到 ROLL_INITIATIVE 重新骰定顺序——至少 3 轮模拟中至少有 1 轮顺序与上一轮不同；(c) 先攻平局按 DEX_mod→DEX_score→LVL→d2 顺序裁决 |
| AC-3 | 反应中断机制在全部 5 种类型中正确触发 | 集成测试 | (a) 借机攻击：敌人离开触及范围时立即中断移动；(b) Shield：被命中时在攻击判定后、伤害前触发；(c) Counterspell：敌人施法时在施法开始后、效果前触发；(d) Protection：邻接队友被攻击时在攻击判定前触发；(e) Uncanny Dodge：被可见攻击者命中时在伤害计算后、伤害应用前触发。所有反应执行后正确恢复原中断点 |
| AC-4 | 分段移动正确支持 | 单元测试 | (a) move→action→move 流程正确执行；(b) 总移动量不超过速度值（含 dash）；(c) 困难地形消耗双倍移动；(d) 速度=0 时无法移动（Grappled/Restrained 正确阻止移动指令） |
| AC-5 | 攻击检定管线完整 | 单元测试 | (a) 近战用 STR（finesse 可选 DEX）、远程用 DEX、法术用施法属性——各 1 例验证；(b) 自然 20=必定命中+暴击；(c) 自然 1=必定未命中；(d) 优势/劣势正确骰 2 次取高/低；(e) 3 源优势+1 源劣势=平骰；(f) 掩体 AC 加成（半掩体+2, 3/4+5）正确应用，多掩体取最高；(g) 长射程劣势正确应用 |
| AC-6 | 伤害计算管线完整 | 单元测试 | (a) 抗性减半（向下取整）——验证 1÷2=0, 15÷2=7；(b) 免疫降为 0；(c) 易伤翻倍；(d) 抗性+易伤=抵消（正常伤害）；(e) 临时HP 先于真实HP 扣除，多源取最高不叠加；(f) 多伤害类型混合攻击（如炽焰长剑 slashing+fire）分别应用独立抗性后合并 |
| AC-7 | 法术位消耗/恢复正确 | 单元测试 | (a) 施法消耗对应环位法术位；(b) 戏法不消耗法术位；(c) 短休恢复一半 1 环位（向上取整，如 4→2, 3→2）；(d) 长休恢复全部法术位；(e) Arcane Recovery 恢复 ceil(Lv/2)环位（验证 Lv5→3环位，Lv7→4环位）；(f) 升环施放消耗高环位（如 Magic Missile 2环→4颗飞弹）；(g) 仪式施法不消耗法术位 |
| AC-8 | 死亡双轨机制正确 | 单元测试 | (a) HP=0 时 rounds_without_healing=0, death_failures=0；(b) 每轮开始时 rounds_without_healing+=1，达到 3→永久死亡；(c) HP=0 时受到任何伤害→death_failures+=2，达到≥3→永久死亡；(d) 两轨同时触发时 death_failures 优先判定；(e) 治疗（HP>0）重置两个计数器为 0；(f) 稳定化（DC 10 Medicine）暂停两个计数器但 HP 仍为 0；(g) Spare the Dying 自动稳定 |
| AC-9 | 撤退机制正常工作 | 集成测试 | (a) 全员同意→进入撤退检定；(b) 多数+领导检定成功(d20+CHA_mod vs DC 12)→进入撤退检定；(c) 撤退 DC 公式正确（验证普通遭遇+出口场景 DC=7，Boss场景 DC=24）；(d) 成功→应用全部撤退惩罚（疲乏+1、金币损失 10-20%、装备 condition降1级）；(e) 失败→全员承受一次借机攻击→战斗继续（不重骰先攻）；(f) 失败后本轮不可再提议撤退 |
| AC-10 | 条件系统正确 | 单元测试 | (a) 14 种条件每种的核心机械效果验证（至少 1 种效果/条件）；(b) 同名条件不堆叠，取最长持续时间；(c) 异名条件共存不冲突；(d) 失能传递链正确（Paralyzed/Stunned→Incapacitated / Unconscious→Incapacitated+Prone）；(e) 条件豁免结束正确（Poisoned 每轮结束 CON 豁免 DC13）；(f) 疲乏 Lv3+再获得疲乏=死亡 |
| AC-11 | AI 目标选择满足启发式排序 | 单元测试 | 给定 3 个预设队伍成员（HP=5/20、HP=15/20且正在专注、HP=20/20）距敌人相同距离：(a) 专注中成员得分最高（+10加成）；(b) 低HP成员得分高于满HP成员；(c) 中毒成员得分降低5分；(d) 评分输出为确定性值（相同输入→相同输出）；(e) AC/命中率考量：当目标 AC 使命中率<30%时得分×0.5 |
| AC-12 | Boss 阶段转换正确 | 集成测试 | (a) HP 首次≤50%时触发阶段2行为变更；(b) 一击跨阶段（从51%→0%）时阶段2逻辑在伤害完全结算后检查——若Boss已死亡则不触发；(c) 再生被火焰/酸蚀抑制时阶段2的"拉开距离等再生"行为替换为"优先攻击持有火/酸伤害来源者" |
...
{
    int RollD20();           // 单次 d20
    int RollDamage(string dice); // 如 "2d6+3"
}

public class MockDiceRoller : IDiceRoller
{
    private readonly Queue<int> _sequence;
    public MockDiceRoller(params int[] sequence) { _sequence = new Queue<int>(sequence); }
    public int RollD20() => _sequence.Count > 0 ? _sequence.Dequeue() : 10;
    public int RollDamage(string dice) => _sequence.Count > 0 ? _sequence.Dequeue() : 4;
}

// 使用示例:
// var dice = new MockDiceRoller(20, 8, 6);  // 自然20暴击 → d8=8 → d6=6
```
...
*文档版本: v2.0*
*创建日期: 2026-05-04*
*最后更新: 2026-05-10（设计评审修订——5阻塞已解决）*
*状态: 待复审 (MAJOR REVISION → 修订完成)*
