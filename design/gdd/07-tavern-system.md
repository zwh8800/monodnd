...
>
> **📜 酒馆设计哲学 (2026-05-09 裁决)**: 酒馆是"家"而非"菜单"。
> - 招募 = 走进酒馆和冒险者对话，而非从列表中选择
> - 商店 = 行商不定期出现，有随机稀有物品和讨价还价
> - 任务 = 吧台旁布告栏，每次回来有新纸条
> - 每次冒险归来触发 1 个简短酒馆事件（LLM 生成 1-2 句叙事 + 轻量效果）
> - 实现策略: 系统架构不改——用叙事包装替代菜单交互。所有底层功能（招募/商店/任务）保持不变，仅改变玩家接触它们的方式。
...
| **空间驱动交互（主），菜单辅助（辅）** | 核心流程（招募、接任务、交易）默认为走进酒馆与 NPC/场景物体交互。菜单列表仅作为快捷操作（键盘热键），方便反复操作时快速访问——类似 Darkest Dungeon 的 Hamlet 操作模式 |
| **程序控制骨骼，LLM绘制皮肤** | 所有数值计算、条件判定、事件触发由程序控制；LLM只生成叙事文本 |
| **渐进解锁** | 酒馆设施、功能区域随酒馆等级逐步解锁，形成长线目标 |
...
| 区域解锁 (铁匠/炼金/图书馆/神殿/英雄之壁) | 部分：**行商铁匠（基础装备维修）** — 铁匠作为行商不定期出现在大厅，无需解锁 Lv2。行商模式是 MVP 期间装备维修的唯一途径 | ✅ |
| 招募系统 (基础版) | ✅ (9个预设角色) | ✅ (全管线) |
| NPC调度系统 | ❌ — MVP 仅固定 NPC（酒馆老板 + 行商铁匠）始终可见，无动态调度 | ✅ |
| 商店系统 (基础版) | ✅ (杂货商 — 大厅固定 NPC) | ✅ (全商店) |
| 酒馆事件系统 | ❌ | ✅ |
| 关系事件系统 | ❌ | ✅ |
| 对话系统架构 | ❌ | ✅ |
| 训练系统 | ❌ | ✅ |
| 任务板系统 | ✅ (基础版 — 吧台旁布告栏) | ✅ (完整版) |
| 休息与恢复机制 | ✅ (基础版) | ✅ (完整版) |
| 角色管理与界面 | ✅ | ✅ |

> **MVP 实施说明**:
> - **交互模式**: MVP 采用混合方案——核心流程（招募→与大厅中可见的预设角色对话；接任务→点击吧台旁布告栏实体；交易→与杂货商/行商铁匠对话）为空间驱动交互。菜单列表仅作为键盘快捷键辅助操作。
> - **酒馆固定 Lv1**: 仅大厅场景可用。铁匠铺/炼金台/图书馆/神殿需要 Lv2+ 解锁，不在 MVP 范围。
> - **招募池**: MVP 使用 9 个预先生成的角色（3 种族 × 3 职业），含固定数值和背景故事，不依赖 LLM 生成管线。
> - **行商铁匠**: 不定期出现在大厅的旅行 NPC（替代 Lv2 铁匠铺），提供装备修复和基础武器购买。

---

## 1A. 玩家体验幻想 (Player Fantasy)

- **"这是我的酒馆"** — 从破旧小屋到金碧辉煌，每次升级都是冒险的成就。
- **"这些人我舍不得"** — 角色在酒馆中闲逛、对话、训练，关系自然发展。
- **"这里是家"** — 温暖酒馆与寒冷地牢形成对比，是安全港湾和冒险起点。
...
| 参数 | 当前值 | 安全范围 | 影响面 |
|------|:------:|:--------:|--------|
| 酒馆等级上限 | 10 | 8-15 | 设施解锁节奏 |
| 招募槽位 | 4 | 3-6 | 队伍规模 |
| 商店刷新 | 每次冒险后 | 即时/周期 | 经济节奏 |
| 事件频率(Lv1) | 1次/2冒险 | 1-3 | 叙事密度 |
...
| LLM网关 | 酒馆事件叙事 | ✅ |
| 冒险生成 | 任务板 | ✅ |
| 战斗系统 | 酒馆被袭事件（§6.2 `evt_tavern_attack`）| ✅ |
| 失败与成长 | 声望体系/酒馆XP权威来源（§2.1, §2.2, §2.3）| ✅ |
| 世界状态 | 声望事件/势力关系影响酒馆事件触发 | ✅ |
...
> **数据权威来源**: 本表与 [failure-growth.md §10.2](08-failure-growth.md) 保持同步。酒馆XP数值以 failure-growth.md 为准。
...
酒馆XP来源于冒险完成和特殊成就。完整规则见 [failure-growth.md §10.2](08-failure-growth.md)。
...
声望是独立于酒馆等级的**0-100分制**社交系统，影响商店折扣、NPC态度、酒馆事件、冒险叙事选项等——但**不参与设施解锁**（设施解锁由酒馆等级控制，见 §2.1）。完整规则见 [failure-growth.md §10.1](08-failure-growth.md)。

| 声望范围 | 等级 | 主要影响 |
|:--------:|------|----------|
| 0-19 | 新手 (Novice) | 基础招募池、标准价格 |
| 20-39 | 知名 (Known) | 商店折扣5%、稀有冒险者出现 |
| 40-59 | 受尊敬 (Respected) | 商店折扣10%、特殊酒馆事件 |
| 60-79 | 著名 (Famous) | 商店折扣15%、更好战利品 |
| 80-100 | 传说 (Legendary) | 商店折扣20%、传奇酒馆事件 |
...
|--------|------|:------:|:--------:|----------|----------|-----|
| `area_hall` | 大厅 (Hall) | Lv1 | 0 | — | **酒馆核心场景**: 冒险者散坐酒桌旁可对话招募；吧台旁布告栏可接冒险任务；炉火旁可休息闲聊；行商不定期出现。所有交互以场景叙事呈现——玩家走进酒馆和角色对话，而非点击菜单。 | 酒馆老板 (固定) |
...
| `area_heroes_wall` | 英雄之壁 (Hero's Wall) | 首次冒险完成 | 0 (自动) | — | **MVP: 冒险日志墙** — 每次冒险后自动记录：冒险名称、结局、存活角色、关键事件摘要（1-2句）。**Phase 2+**: 扩展为完整传记生成、传承装备展示、前代冒险者雕塑 | 无固定NPC |

### 2.6 升级资源需求明细

酒馆升级需消耗金币和材料，且必须达到对应累计酒馆XP。累计酒馆XP 数值以 §2.1 主表为准，声望系统（§2.3 的 0-100 分制）独立运作，用于折扣/NPC 态度/事件触发，不参与设施解锁。

| 升至Lv | 金币 | 材料 | 前置条件 |
|:------:|:----:|------|----------|
| Lv2 | 300 | 木材×30, 铁锭×5 | 累计酒馆XP 500 |
| Lv3 | 1,200 | 石材×50, 铁锭×15, 皮革×10 | 累计酒馆XP 1,700 |
| Lv4 | 3,000 | 石材×80, 铁锭×30, 水晶粉×5 | 累计酒馆XP 4,200, 完成≥3次冒险 |
| Lv5 | 9,000 | 古代典籍×2, 水晶粉×15, 魔法墨水×5 | 累计酒馆XP 8,700 |
| Lv6 | 15,000 | 石材×150, 水晶粉×25, 精华×5 | 累计酒馆XP 16,200, 拥有≥1个Lv5角色 |
| Lv7 | 24,000 | 圣水×5, 银锭×15, 古代典籍×5 | 累计酒馆XP 28,200 |
| Lv8 | 38,000 | 龙鳞碎片×3, 秘银锭×10, 精华×15 | 累计酒馆XP 46,200, 完成≥1次中冒险 |
| Lv9 | 60,000 | 龙鳞碎片×10, 秘银锭×25, 传奇精华×1 | 累计酒馆XP 72,200 |
| Lv10 | 100,000 | 龙鳞碎片×25, 传奇精华×5, 龙心×1 | 累计酒馆XP 108,200, 完成≥1次长冒险 |
...
Step 5: 计算招募费
  招募费 = max(50, base_cost + level_cost)
  base_cost = 100 GP × 候选人等级 (反映角色内在价值)
  level_cost 规则:
    候选人等级 > 玩家等级: +80 GP × 等级差
    候选人等级 = 玩家等级: 0 GP
    候选人等级 < 玩家等级: -40 GP (固定减免，不随等级差递增)
    例: Lv2 候选人, Lv3 玩家 → 200 + (-40) = 160 GP
    例: Lv5 候选人, Lv5 玩家 → 500 + 0 = 500 GP
    下界钳制: 最终招募费 ≥ 50 GP (见 §3.6 JSON Schema)
  设计意图: 随着酒馆声望提升，高级冒险者慕名而来——他们的身价也更高
...
> **公式**: `recruit_cost = max(50, 100 × candidate_level + level_cost)`
> - 高于玩家: `level_cost = +80 GP × 等级差`
> - 等于玩家: `level_cost = 0 GP`
> - 低于玩家: `level_cost = -40 GP`（固定减免）

| 角色等级 | 玩家等级 | 价差 | 基础费 | 调整 | 总招募费 |
|:--------:|:--------:|:----:|:------:|:----:|:--------:|
| Lv1 | Lv1 | 0 | 100 | 0 | **100 GP** |
| Lv1 | Lv3 | -2 | 100 | -40 | **60 GP** |
| Lv2 | Lv1 | +1 | 200 | +80 | **280 GP** |
| Lv3 | Lv1 | +2 | 300 | +160 | **460 GP** |
| Lv3 | Lv3 | 0 | 300 | 0 | **300 GP** |
| Lv4 | Lv3 | +1 | 400 | +80 | **480 GP** |
| Lv5 | Lv5 | 0 | 500 | 0 | **500 GP** |
| Lv8 | Lv8 | 0 | 800 | 0 | **800 GP** |
...
| 等级训练 | 训练师NPC | Lv5 | 金币 + 材料 | 1天/级 | 见表9.5 |
...
> **设计原则**: 2026-05-09 `/review-all-gdds` 裁决——删除金币→XP购买机制。该机制直接违反"NOT 刷关打宝"反支柱（`design/pillars.md`）。经验值只能通过冒险获取，训练系统仅用于技能/专长/属性的金币→能力转换（见 §9.1-9.4）。
...
| 复活角色 | 复活死亡角色 (死亡≤7天) | 2,000 | Lv7 | 钻石×1 (价值500 GP) |
| 高等复活 | 复活死亡角色 (死亡≤30天) | 5,000 | Lv9 | 钻石×2 (价值2,000 GP) |
...
```
TEST 1: 酒馆XP阈值驱动等级变化
  输入: current_tavern_xp = 3,700
  预期: GetTavernLevel(3,700) == 3 (刚好在Lv3，见 §2.1 累计酒馆XP列)
  输入: current_tavern_xp = 3,701
  预期: GetTavernLevel(3,701) == 4 (跨越到Lv4)
  输入: current_tavern_xp = 254,700
  预期: GetTavernLevel(254,700) == 10 (Lv10 上限)
  输入: current_tavern_xp = 999,999
  预期: GetTavernLevel(999,999) == 10 (不再增长，已到上限)
  验证: GetTavernLevel() 返回值永远在 [1, 10] 区间

TEST 2: 区域解锁条件验证
  输入: tavern_level = 1, 尝试解锁 area_blacksmith
  预期: CanUnlockArea("area_blacksmith") == false (酒馆等级不足)
  输入: tavern_level = 2, player_gold = 500, 材料充足, 尝试解锁 area_blacksmith
  预期: CanUnlockArea("area_blacksmith") == true, 解锁后 gold → 0, 材料扣除

TEST 3: 冒险报酬计算（跨系统依赖: failure-growth.md §10.2）
  注意: 难度倍率和基础XP值定义于 failure-growth.md，本测试仅验证酒馆端计算
  输入: 短冒险, difficulty_multiplier = 1.3 (Hard), base_tavern_xp = 300
  预期: earned_xp = floor(300 × 1.3) = 390
  输入: 长冒险, difficulty_multiplier = 1.6 (Deadly), base_tavern_xp = 2,500
  预期: earned_xp = floor(2,500 × 1.6) = 4,000
  验证: earned_xp 为严格正整数，≥ 最小门槛值
```
...
```
TEST 22: 训练费用验证
  输入: train_type = "skill", skill_id = "athletics"
  预期: cost = 300 GP, training_days = 3
  来源: §9.2 技能训练费用表

TEST 23: 训练限制 [原 TEST 24]
  输入: 同一天训练2次
  预期: 第二次训练被拒绝
  验证: IsTrainingAvailable(character_id) == false 当有训练进行中时

TEST 24: 训练事件触发 [NEW]
  输入: 训练完成 (training_type 任意)
  预期: TrainEventRoll(chance=15%) 可能触发四种事件之一:
    - 突破性领悟: XP额外 +10%
    - 意外受伤: training_days += 1
    - 导师赏识: relationship += 2 (仅NPC训练)
    - 灵感迸发: 获得临时技能加成
  验证: 最多触发一个事件，概率总和 15%
```
